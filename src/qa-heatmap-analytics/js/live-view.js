...
qahmz.liveView.safeDecodeComponent = function(str) {
	try {
...
// #1512: separate キー（media_source_campaign_goal の4成分）を分解する。
// 旧形式（成分未エンコード）のキーは成分内の「_」で 4 要素にならないことがあり、その場合は null（分解不能）を返す。
qahmz.liveView.parseSeparateKey = function(key) {
	var parts = key.split('_');
	if (parts.length !== 4) {
		return null;
	}
	return {
		media: qahmz.liveView.safeDecodeComponent(parts[0]),
		source: qahmz.liveView.safeDecodeComponent(parts[1]),
		campaign: qahmz.liveView.safeDecodeComponent(parts[2]),
		goal: qahmz.liveView.safeDecodeComponent(parts[3])
	};
};
...
qahmz.liveView.processSeparateData = function(separateData) {
	...
	var collect = function(key) {
		...
		// #1512: 分解不能な旧形式キーはキー一覧（filterKeyAry）には保持するが、フィルタ候補には混ぜない
		var parsed = qahmz.liveView.parseSeparateKey(key);
		if (!parsed) {
			return;
		}
		if (qahmz.liveView.filterSourceAry.indexOf(parsed.source) === -1) {
			qahmz.liveView.filterSourceAry.push(parsed.source);
		}
		if (qahmz.liveView.filterMediaAry.indexOf(parsed.media) === -1) {
			qahmz.liveView.filterMediaAry.push(parsed.media);
		}
		if (qahmz.liveView.filterCampaignAry.indexOf(parsed.campaign) === -1) {
			qahmz.liveView.filterCampaignAry.push(parsed.campaign);
		}
		// #1512: Goal 候補も実データから生成する（旧実装は '○'/'×' 固定＝実値 '(不明)' が選択不能だった）
		if (qahmz.liveView.filterGoalAry.indexOf(parsed.goal) === -1) {
			qahmz.liveView.filterGoalAry.push(parsed.goal);
		}
	};
...
qahmz.liveView.applyFilter = function() {
	...
	for (var i = 0; i < allKeys.length; i++) {
		// #1512: 分解はヘルパへ一元化（decode 込み）。分解不能な旧形式キーはフィルタ選択時のマッチ対象外。
		var parsed = qahmz.liveView.parseSeparateKey(allKeys[i]);
		if (!parsed) { continue; }

		if (selSource && parsed.source !== selSource) { continue; }
		if (selMedia && parsed.media !== selMedia) { continue; }
		if (selCampaign && parsed.campaign !== selCampaign) { continue; }
		if (selGoal && parsed.goal !== selGoal) { continue; }
...
qahmz.liveView.renderer.renderScrollMap = function() {
	...
	// #1512: フィルタ0件等でデータが無いときは、直前の描画を残さずクリアしてから戻る
	if (!qahmz.liveView.blockAry) {
		target.innerHTML = '';
		return;
	}
...
qahmz.liveView.renderer.renderClickHeatMap = function() {
	...
	if (!target || !qahmz.liveView.data) {
		return;
	}
	// #1512: フィルタ0件等でクリックデータが無いときは、直前の描画を残さずクリアしてから戻る
	if (!qahmz.liveView.data.merge_c) {
		target.innerHTML = '';
		return;
	}
