...
class QAHM_Behavioral_Data extends QAHM_File_Data {
	...
	public function init_session_data( $qa_id, $title, $url, $c_url, $url_hash, $ref, $country, $ua, $tracking_id, $is_new_user, $is_cookie_reject, $ip_address, $original_id = '' ) { //QA ZERO add T50: $original_id追加
	//public function init_session_data( $qa_id, $wp_qa_type, $wp_qa_id, $title, $url, $ref, $country, $ua ) { QA ZERO del
			...
			$utm_content      = '';
			$utm_term         = '';
			$user_original_id = substr( str_replace( array( "\t", "\n", "\r" ), '', $original_id ), 0, 191 ); // T50: 多重ガード（入口はqahm-ajax.php側で正規化済み）
	...
	/**
	 * クリック先 URL（TRANSITION）の大文字小文字を正規化する。#1603
	 *
	 * 内部リンクは従来どおり小文字化し、外部リンクは原文のまま残す。
	 *
	 * 従来はここで無条件に小文字化していたため、大文字小文字が意味を持つ外部 URL
	 * （例: youtube.com/channel/UCxxxx）が壊れ、リンクとして開けなくなっていた。
	 * 小文字化そのものは自サイト URL をクリック先辞書の中で名寄せする働きがあるが、
	 * 他サイトの URL にその利益は無く、壊すだけになる。
	 *
	 * 内部判定は夜間 cron の is_external と同一の規則を使う
	 * （class-qahm-columndb-cron.php ＝ 相対パス、または https:// / http:// ＋ドメインの前方一致の3条件）。
	 * 判定を1つの規則に揃えることで「受信時は内部・cron では外部」というズレを作らない。
	 * 判定から漏れた場合は「外部＝原文のまま」へ倒れるので、データを壊す側には転ばない。
	 *
	 * なお内部リンクをサイト設定（url_case_sensitivity）に従わせるかは本修正では扱わない。
	 * 設定を尊重すると QA ZERO の登録既定（1＝区別する）では内部リンクの記録も変わり、
	 * 影響が普段の導線まで広がるため、次版で別途扱う。
	 *
	 * @param mixed  $transition  クリック先 URL。文字列以外・空文字はそのまま返す（従来の mb_strtolower も文字列前提だった）。
	 * @param string $site_domain 計測サイトのドメイン（qahm_sitemanage の domain ＝ホスト名のみ）。
	 * @return mixed 正規化後のクリック先 URL。入力が文字列以外・空文字のときは入力をそのまま返すため string 固定ではない。
	 */
	private function normalize_click_transition_case( $transition, $site_domain ) {
		if ( ! is_string( $transition ) || '' === $transition ) {
			return $transition;
		}

		// 内部判定＝cron の is_external と同一規則。
		$is_internal = ( 0 === strpos( $transition, '/' ) );
		if ( ! $is_internal && '' !== $site_domain ) {
			$is_internal = ( 0 === strpos( $transition, 'https://' . $site_domain )
				|| 0 === strpos( $transition, 'http://' . $site_domain ) );
		}
		...
		return mb_strtolower( $transition );
	}

	public function record_behavioral_data($is_pos, $is_click, $is_event, $is_dLevent, $raw_name, $readers_name, $ua, $tracking_id, $url_hash, $is_cookie_reject, $site_domain = '' ) {
		...
		try {
			...
			if ( $is_pos ) {
				...
				// T108: このPVで submit イベントが観測されたか（qtag のPVフラグ由来）。
				// raw_p ヘッダーに載せ、cron が is_submit をここから直接得る（action_id 非依存）。
				$is_submit = $this->wrap_filter_input( INPUT_POST, 'is_submit' );
				$is_submit = ( '1' === (string) $is_submit ) ? 1 : 0;
				$pos_ary  = array(
					array(
						self::DATA_HEADER_VERSION       => $pos_ver,
						self::DATA_POS_HEADER_IS_SUBMIT => $is_submit,
					)
				);
			...
			if ( $is_click ) {
				...
				for ( $i = 0, $click_ary_cnt = $this->wrap_count( $click_ary ); $i < $click_ary_cnt; $i++ ) {
					...
					if( $this->wrap_array_key_exists( self::DATA_CLICK_1['TRANSITION'], $click_ary[$i] ) ) {
						// #1603: 内部リンクは従来どおり小文字化し、外部リンクは原文のまま残す。
						// 従来はここで無条件に mb_strtolower していたため、外部リンクの
						// 大文字小文字が失われ、リンクとして開けない URL が記録されていた。
						$click_ary[$i][self::DATA_CLICK_1['TRANSITION']] = $this->normalize_click_transition_case(
							$click_ary[$i][self::DATA_CLICK_1['TRANSITION']],
							$site_domain
						);
					}
