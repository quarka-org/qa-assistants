...
if ( $get_action === 'get_live_view_data' ) {
    header( 'Content-Type: application/json; charset=utf-8' );

    $token = isset( $_GET['token'] ) ? sanitize_text_field( wp_unslash( $_GET['token'] ) ) : '';
    if ( empty( $token ) ) {
        echo wp_json_encode( array( 'success' => false, 'data' => 'Missing token' ) );
        exit;
    }

    require_once 'class-qahm-view-base.php';
    require_once 'class-qahm-view-heatmap.php';

    global $qahm_view_heatmap;
    if ( ! $qahm_view_heatmap ) {
        $qahm_view_heatmap = new QAHM_View_Heatmap();
    }
    $qahm_view_heatmap->init_wp_filesystem();

    $data = $qahm_view_heatmap->load_live_view_data( $token );
    if ( $data === null ) {
        echo wp_json_encode( array( 'success' => false, 'data' => 'Invalid or expired token' ) );
        exit;
    }

    $token_page_url = isset( $data['_token_page_url'] ) ? $data['_token_page_url'] : '';
    unset( $data['_token_page_url'] );

    if ( ! empty( $token_page_url ) ) {
        // Plugin Check exclusion: wp_parse_url() not available in SHORTINIT context; safe fallback to parse_url()
        // phpcs:ignore WordPress.WP.AlternativeFunctions.parse_url_parse_url
        $origin_parts = parse_url( $token_page_url );
        if ( isset( $origin_parts['scheme'], $origin_parts['host'] ) ) {
            $allowed_origin = $origin_parts['scheme'] . '://' . $origin_parts['host'];
            if ( ! empty( $origin_parts['port'] ) ) {
                $allowed_origin .= ':' . $origin_parts['port'];
            }
...
// 計測サイト未登録時 get_option() は false を返す。配列関数へ生で渡すと PHP8 では TypeError になる（#1548）
...
switch ( $action ) {
    ...
    case 'init_session_data':
        ...
        // T49: $_COOKIEからqa_id_zを優先読み取り（サーバーサイドCookie）
        $cookie_qa_id = isset( $_COOKIE['qa_id_z'] ) ? $_COOKIE['qa_id_z'] : '';
        $is_new_user  = 0;

        // バリデーション: 28桁英数字のみ許可（preg_match禁止ルール準拠）
        if ( $cookie_qa_id !== '' ) {
            if ( strlen( $cookie_qa_id ) !== 28 || ! ctype_alnum( $cookie_qa_id ) ) {
                $cookie_qa_id = '';
            }
        }

        if ( $cookie_qa_id !== '' ) {
            $qa_id = $base->serialize_escape( $cookie_qa_id );
        } else {
            $is_new_user = 1;
            $anontrack = false;
        ...
        // T49: サーバー側で$_COOKIEを確認し、同意済みならis_rejectを上書き
        // （JSはHttpOnly Cookieを読めないため、初回は正確なis_rejectを送れない）
        $is_consented = isset( $_COOKIE['qa_cookieConsent'] ) && $_COOKIE['qa_cookieConsent'] === 'true';
        if ( $is_consented ) {
            $is_reject = false;
        }

        // T50: original_idをPOSTから取得（入口で正規化・サニタイズを完結）
        // serialize_escapeは使わない（外部IDの:{}等がエスケープされると外部システムとの照合不可）
        $original_id = '';
        $original_id_raw = $base->wrap_filter_input( INPUT_POST, 'original_id' );
        if ( $original_id_raw !== null && $original_id_raw !== '' ) {
            // wrap_trim() は protected のため、クラス外であるこのファイルからは呼べない（#1550）
            $original_id = trim( $original_id_raw );
            $original_id = str_replace( array( "\t", "\n", "\r" ), '', $original_id );
            $original_id = substr( $original_id, 0, 191 );
        }

        //init
        $data = $behave->init_session_data( $qa_id, $title, $url, $c_url, $url_hash, $ref, $country, $ua, $tracking_id, $is_new_user, $is_reject, $ip_address, $original_id ) ;
        ...
        if ( ! $is_reject ) {
            qahm_set_server_cookie( 'qa_id_z', $qa_id, $two_years, $is_own_domain, $orgin );
        } else {
            // Cookie拒否時は既存Cookieを削除
            qahm_delete_server_cookie( 'qa_id_z', $is_own_domain, $orgin );
            qahm_delete_server_cookie( 'qa_cookieConsent', $is_own_domain, $orgin );
        }

        // T49: 同意状態をレスポンスに含める（JSがHttpOnly Cookieを読めないため）
        $data['is_consented'] = $is_consented;

        qahm_return_json( $data );
        break;

    case 'cookie_consent':
        // T49: 同意タグからのAjax — qa_cookieConsent + qa_id_z を同時発行
        $ua = $base->serialize_escape( $base->wrap_filter_input( INPUT_SERVER, 'HTTP_USER_AGENT' ) );
        $two_years = 60 * 60 * 24 * 365 * 2;

        // qa_cookieConsent Cookie発行
        qahm_set_server_cookie( 'qa_cookieConsent', 'true', $two_years, $is_own_domain, $orgin );

        // qa_id生成: cookie_qa_id_mode設定で分岐
        $cookie_qa_id_mode = isset( $site[0]['cookie_qa_id_mode'] ) ? $site[0]['cookie_qa_id_mode'] : 'unique';
        $existing_qa_id = isset( $_COOKIE['qa_id_z'] ) ? $_COOKIE['qa_id_z'] : '';

        // 既存Cookieが有効ならそのまま使用
        if ( $existing_qa_id !== '' && strlen( $existing_qa_id ) === 28 && ctype_alnum( $existing_qa_id ) ) {
            $qa_id = $existing_qa_id;
        } else {
            if ( $cookie_qa_id_mode === 'anonymous_continue' ) {
                $anontrack = false;
                if ( isset( $site[0]['anontrack'] ) ) {
                    $anontrack = $site[0]['anontrack'];
                }
        ...
        qahm_return_json( array( 'success' => true, 'qa_id' => $qa_id ) );
        break;

    case 'cookie_consent_revoke':
        // T49: 同意取り消し — qa_cookieConsent + qa_id_z を削除
        qahm_delete_server_cookie( 'qa_cookieConsent', $is_own_domain, $orgin );
        qahm_delete_server_cookie( 'qa_id_z', $is_own_domain, $orgin );
        qahm_return_json( array( 'success' => true ) );
        break;
    ...
    case 'record_behavioral_data':
        ...
        // #1062: init_session_dataと同じく、サーバー側で同意Cookieを確認しis_rejectを上書き
        // （同一オリジン計測ではbehavioralリクエストにもCookieが乗るため、
        //   旧キャッシュのqtag.jsがis_reject=trueを送り続けても同意済みユーザーを誤分類しない。
        //   クロスドメイン計測はwithCredentialsなしでCookie不達のため従来どおりJS値を使う）
        // 上書きは true→false の一方向のみ（同意Cookieが在る時だけ拒否を解除）。逆向き（false→true）は行わない。
        //   同意取り消しは cookie_consent_revoke が qa_cookieConsent を即削除するため、
        //   次回リクエストでは $_COOKIE が消えて自然に拒否へ戻る（revoke競合の懸念なし）。
        $is_consented = isset( $_COOKIE['qa_cookieConsent'] ) && $_COOKIE['qa_cookieConsent'] === 'true';
        if ( $is_consented ) {
            $is_reject = false;
        }
        ...
        // #1603: クリック先 URL の内部/外部を判定するためのドメインを、ページ URL 側と同じ $site[0] から渡す。
        // 引き直さずに渡すのは、cron の is_external と別の行を見てしまう事故を構造的に防ぐため。
        $site_domain = isset( $site[0]['domain'] ) ? $site[0]['domain'] : '';
        $output = $behave->record_behavioral_data( $is_pos, $is_click, $is_event, $is_dLevent, $raw_name, $readers_name, $ua, $tracking_id, $url_hash, $is_reject, $site_domain ); //QA ZERO add
...
/**
 * サーバーサイドCookie発行（T49）
 * qa_id_z / qa_cookieConsent をSet-Cookieヘッダーで発行する
 */
function qahm_set_server_cookie( $name, $value, $max_age, $is_own_domain, $origin = '' ) {
    $secure   = is_ssl();
    $samesite = 'Lax';
    $domain   = '';
    ...
    if ( ! $is_own_domain ) {
        $secure   = true;
        $samesite = 'None';
        // Originからホストを取得してccTLDロジックでdomain属性を計算
        if ( $origin !== '' ) {
            // Plugin Check exclusion: wp_parse_url() not available in SHORTINIT context
            // phpcs:ignore WordPress.WP.AlternativeFunctions.parse_url_parse_url
            ...
            if ( $host ) {
                ...
                if ( $count >= 3 ) {
                    ...
                    if ( in_array( $last_two, $cc_tlds, true ) ) {
                        $domain = '.' . $host_parts[ $count - 3 ] . '.' . $last_two;
                    } else {
                        $domain = '.' . $last_two;
                    }
                } else {
 ...
 * サーバーサイドCookie削除（T49）
 */
function qahm_delete_server_cookie( $name, $is_own_domain, $origin = '' ) {
    qahm_set_server_cookie( $name, '', -86400, $is_own_domain, $origin );
}
...
function qahm_get_domains_with_cache() {
    ...
    // 計測サイトが1件も無い場合 $domains は false のまま＝呼び出し元の array_merge() が落ちるため、必ず配列で返す（#1548）
