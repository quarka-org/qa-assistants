...
try {
	...
	foreach ( $content_info_ary as $content_info ) {
		// #1512: 値（separate_data_num の JSON 等）に「=」が含まれても壊れないよう、最初の「=」でのみ分割する
		$exp_info = explode( '=', $content_info, 2 );
		switch ( $exp_info[0] ) {
			case 'data_num':
	...
	<head>
		...
		<script>
			...
			var qahml10n = {
				...
				// 絞り込みウィンドウ
				'source': '<?php echo esc_html( qahm_get_utm_label( 'source' ) ); ?>',
				'medium': '<?php echo esc_html( qahm_get_utm_label( 'medium' ) ); ?>',
				'campaign': '<?php echo esc_html( qahm_get_utm_label( 'campaign' ) ); ?>',
				'goal': '<?php echo esc_html__( 'Goal', 'qa-heatmap-analytics' ); ?>',
				'select_source': '<?php echo esc_html( qahm_get_utm_select_label( 'source' ) ); ?>',
				'select_medium': '<?php echo esc_html( qahm_get_utm_select_label( 'medium' ) ); ?>',
				'select_campaign': '<?php echo esc_html( qahm_get_utm_select_label( 'campaign' ) ); ?>',
