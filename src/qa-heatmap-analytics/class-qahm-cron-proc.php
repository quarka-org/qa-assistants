		...
		while ( $while_continue ) {
			...
			switch ( $cron_status ) {
				...
				case 'Night>Make view file>Version_hist>Start':
					...
					// phpcs:disable WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- table names from $wpdb->prefix, not user input.
					$result = $wpdb->get_results(
						"
...
// koji maruyama end
					...
					$result = $wpdb->get_results(
						...
						if ( ! empty( $result ) ) {
							...
							foreach ($result as $idx => $row ) {
								...
								// raw_g: is_raw_gフラグがないため、ファイル存在チェックで判定
								$g_str   = '';
								...
								if ( $wp_filesystem->exists( $file_g ) ) {
									...
									foreach ( $raw_g_data_ary as $raw_g_data ) {
										if ( (int) $newary[$idx]['pv_id'] !== (int) $raw_g_data['pv_id'] ) {
											continue;
										}
								...
								if ( ! empty( $g_str ) ) {
									$raw_g_ary[]                 = ['pv_id' => $row->pv_id, 'raw_g' => $g_str];
									$t_raw_g_ary[$tracking_id][] = ['pv_id' => $row->pv_id, 'raw_g' => $g_str];
									$delete_rawfiles_ary[]       = $file_g;
								}
							}
					...
					if( $this->wrap_count( $reqfiles ) == 0 && $this->wrap_count( $proc_reqfiles ) == 0 && $this->wrap_count( $done_reqfiles ) > 0 ){
						...
						//依頼処理結果ファイルを結合
						$types = ['view_pv', 'raw_p', 'raw_c', 'raw_e', 'raw_g'];  // 処理するファイルタイプ
