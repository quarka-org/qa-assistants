...
class QAHM_Cron_Proc extends QAHM_File_Data {
    ...
    // P7-D PR2 #1352: raw_x 日次集約の独立ステップ gating。既定 false で従来フロー完全不変（本番は false 固定・検証時のみ true）。
    const PR2_RAWX_STAGE_ENABLED   = false;
    // P7-D PR3 #1379: raw_x 集約を view_pv から分離し独立ステップを本番正本化する cutover gating。
    // 既定 false で従来フロー完全不変（view_pv が raw_x を生成）。true で view_pv は raw_x を作らず、独立ステップが
    // 本番 raw_X/ を生成（既存日次を読む→日付globで旧日次を全削除→新名で本番へ書く）＋ -done 削除を担う。
    // 本番への実 flip は P7-X 直前/別合図。dev5 検証は true で end-to-end（過去日 rebuild 含む）。
    const PR3_RAWX_CUTOVER_ENABLED = true;
	...
	public function data_manage() {
		...
		$caller = $backtrace[0];
		$qahm_log->debug( "Called from: " . $caller['file'] . " on line " . $caller['line'] );
		$qahm_log->debug( QAHM_NAME . ' cron_data_manage start ' . $this->get_data_dir_path() );
		...
		$cron_step_array = $this->wrap_explode( '>', $cron_status );
		...
		while ( $while_continue ) {
			$qahm_log->debug( 'cron_status:' . $cron_status );
			switch ( $cron_status ) {
				case 'Cron start':
					$this->backup_prev_status( $cron_status );
					// ---next
					$cron_status = 'Common>Check base dir';
					$this->set_next_status( $cron_status );
					break;


				case 'Common>Check base dir':
					$this->backup_prev_status( $cron_status );
					...
					if ( ! $wp_filesystem->exists( $raw_g_dir ) ) {
						$wp_filesystem->mkdir( $raw_g_dir );
					}
					// reader
					...
					// ---next
					$cron_status = 'Common>Check update';
					$this->set_next_status( $cron_status );
					break;

				case 'Common>Check update':
					$this->backup_prev_status( $cron_status );
				...
				case 'Common>Check free':
					$this->backup_prev_status( $cron_status );

					$cron_status = 'Common>Check license';
					$license_authorized = $this->wrap_get_option( 'license_authorized' );
					// ライセンス認証前は、ライセンスチェックをスキップ
					if ( ! $license_authorized ) {
						$cron_status = 'Common>Session check';
					}
					// ---next
					$this->set_next_status( $cron_status );
					break;

				case 'Common>Check license':
					$this->backup_prev_status( $cron_status );
				...
				// ----------
				// Common (Day/Night 判定前に毎サイクル実行)
				// ----------
				case 'Common>Session check':
					$this->backup_prev_status( $cron_status );
					...
					if ( $session_files ) {
						...
						if ( $count_pv >= $limit_pv ) {
							...
							foreach ( $session_files as $session_file ) {
								...
								if ( $elapsed_sec > ( $min * 60 ) ) {
									...
									for ( $iii = 0; $iii < $readers_temp_body_max; $iii++ ) {
										...
										$raw_g_path = $raw_dir . $qa_id_ary[0] . '_' . $access_time . '-g.php';
										$raw_g_tsv = null;
										if ( $wp_filesystem->exists( $raw_g_path ) ) {
											$raw_g_tsv = $this->wrap_delete( $raw_g_path );
										}
									}
							...
							// ---next
							// PV超過でもNight処理は実行すべきなので、Day>End ではなく Check time に遷移
							$cron_status = 'Common>Check time';
							$this->set_next_status( $cron_status );
							break;
						...
						foreach ( $session_files as $session_file ) {
							...
							if ( $elapsed_sec > ( $min * 60 ) ) {
								...
								if ( $readers_finish_ary ) {
									...
									$session_name_parts = self::parse_session_filename( $session_file['name'] );
									if ( $session_name_parts === null ) {
										//例外
									...
									$base_name = $session_name_parts[0];
									$default_session_number = (int) $session_name_parts[1];
				...
				// HTML定期取得（ZERO専用）
				// ----------
				// [Issue #1572] HTML定期取得の3ステージは try/finally で囲み、例外時も遷移先が
				// 必ず書かれるようにする。set_next_status() に到達しないと cron_status が同一
				// ステージのまま永続化され、次回起動も同じ場所で落ちて自力復帰できなくなる。
				// 例外はそのまま再送出され、cron_data_manage() の catch でログに残る。
				// 例外時の遷移先は3ステージとも Day>End とする。false 時の遷移先（Organize →
				// Create の作り直し）を例外にも使うと、恒久的な例外では Create ↔ Organize を
				// 1起動内で MAX_WHILECOUNT 回まで往復してしまうため。
				case 'Day>HTML periodic>Create':
					...
					try {
						...
						// $state === false: 対象サイトなし or エラー → スキップ（Day>End のまま）
					} finally {
						$this->set_next_status( $cron_status );
					}
					break;
				...
				case 'Night>Create yesterday data>Start':
					...
					if ( is_array( $fin_session_files ) ) {
						...
						foreach ( $fin_session_files as $fin_session_file ) {
							$fname_date_str = self::find_yyyy_mm_dd_segment( $fin_session_file['name'] );
				...
				case 'Night>Create yesterday data>Loop>Make Array':
					...
					if ( $iii > 0 ) {
						...
						for ( $iii = $nowloop; $iii < $thisloopmax; $iii++ ) {
							...
							$parsed_session_no = self::parse_trailing_session_no( $yday_filename_str );
							$session_no  = ( $parsed_session_no === null ) ? '' : $parsed_session_no;
					...
					if ( ! empty( $data_ary ) ) {
							...
							if ( ! $is_first_line ) {
								...
								if ( ! empty( $qa_id ) ) {
									...
									if ( ! empty( $result ) ) {
										...
										$table_name = $wpdb->prefix . 'qa_readers';
										// T50: original_idが空なら既存値を保持（nullガード）
										if ( $original_id !== '' ) {
											$result = $wpdb->query(
												$wpdb->prepare(
											...
											$result = $wpdb->query(
												$wpdb->prepare(
													"UPDATE `{$table_name}` SET update_date = CURDATE() WHERE reader_id = %d",
													$reader_id
												)
											);
										}
						...
						if ( ! empty( $arrayValues ) ) {
							...
							$sql = 'INSERT INTO ' . $table_name . ' ' .
								...
								'original_id = IF(VALUES(original_id) != \'\', VALUES(original_id), original_id), update_date = CURDATE()'; // T50: nullガード
				...
				case 'Night>Create yesterday data>Loop>Insert>Pages':
					...
					if ( ! empty( $data_ary ) ) {
						...
						// [Issue #1239] get_sitemanage() は新規環境で false/空を返すため (array) で防御（cf. L1186 の ! empty ガード）
						foreach ( (array) $siteary as $site ) {
							if ( $site['status'] == 255 ) {
								continue;
							}
				...
				case 'Night>Make view file>Version_hist>Update selector':
					...
					if ( ! empty( $data_ary ) ) {
						...
						$prev_page_id = 0;
						$recent_vid = [];
						$prev_tracking_id_for_selectors = '';
						...
						for ( $iii = $loop_start; $iii < $loop_end; $iii++ ) {
							...
							// tracking_id変更時にグローバルセレクタ辞書を切り替え
							if ( $prev_tracking_id_for_selectors !== $tracking_id ) {
								if ( $global_selectors !== null ) {
									$global_selectors->close();
								}
								$global_selectors = new QAHM_ColumnDB_Selectors( $tracking_id );
								$prev_tracking_id_for_selectors = $tracking_id;
							}
							...
							if ( $prev_page_id != (int)$page_id ) {
								...
								if ( ! empty( $version_id_ary ) ) {
									...
									// base_selectorの読み込みは不要（gXX形式でグローバルセレクタ辞書に移行済み）
								}
							}
							...
							if ( $wp_filesystem->exists( $file_c ) && ! empty( $recent_vid ) ) {
								...
								// raw_cファイルの全行を確認し、セレクタをgXX形式に変換しながらc_strを作っていく
								$c_str = '';
								if ( ! empty( $c_ary ) ) {
									...
									for ( $jjj = self::HEADER_ELM; $jjj < $max_c_no; $jjj++ ) {
										if ( $jjj == self::HEADER_ELM ) {
											...
											// グローバルセレクタ辞書でgXX形式IDに置換
											$global_selector_id = $global_selectors->get_or_create( $selector );
											$c_line_ary[0] = 'g' . $global_selector_id;
				...
				case 'Night>Make view file>View_pv>Make loop':
					...
					if ( $all_temp_files !== false ) {
						foreach ( $all_temp_files as $temp_file ) {
							...
							$reqfile_date = self::parse_viewpv_proc_request( $filename );
							if ( $reqfile_date !== null ) {
								$reqfiles[] = array( 'path' => $temp_dir . $filename, 'date' => $reqfile_date );
							}

							$proc_reqfile_pid = self::parse_processing_by( $filename );
							if ( $proc_reqfile_pid !== null ) {
								$proc_reqfiles[ $proc_reqfile_pid ] = $temp_dir . $filename;
							}

							if ( self::is_done_by_format( $filename ) ) {
								$done_reqfiles[] = $temp_dir . $filename;
							}

							$procfile_pid = self::parse_view_pv_process( $filename );
							if ( $procfile_pid !== null ) {
					...
					foreach ( $proc_reqfiles as $r_qaz_pid => $proc_reqfile ) {
						...
						if ( !$this->wrap_array_key_exists( $r_qaz_pid , $procfiles ) ){
							...
							$proc_reqfile_newname = self::strip_processing_by( $proc_reqfile_name );
							...
							$reqfile_date = self::parse_viewpv_proc_request( $proc_reqfile_newname );
							if ( $reqfile_date !== null ) {
					...
					if( $this->wrap_count( $reqfiles ) == 0 && $this->wrap_count( $proc_reqfiles ) == 0 && $this->wrap_count( $done_reqfiles ) == 0 ){ //依頼ファイルが存在するか
						...
						//依頼ファイル処理結果の収集
						$types = ['raw_p', 'raw_c', 'raw_e', 'raw_g', 'view_pv'];  // 処理するファイルタイプ
						$base_dirs = [ $view_dir ];
...
// P7-D PR3 #1379: cutover ON では旧 raw_x 日次を view_pv 側で消さない（独立ステップが source に読んでから自分で削除する）。
                                if ( ! self::PR3_RAWX_CUTOVER_ENABLED ) {
                                ...
                                if ( $wp_filesystem->exists( $raw_g_dir . $delete_base . 'rawg.php' ) ) $delete_rawx_files[] =  $raw_g_dir . $delete_base . 'rawg.php';
                                }
// koji maruyama end
                            }
...
// P7-D PR3 #1379: cutover ON では旧 raw_x 日次を view_pv 側で消さない（独立ステップが source に読んでから自分で削除する）。
                                    ...
                                    if ( $wp_filesystem->exists( $view_dir . $tracking_id . '/view_pv/raw_g/' . $delete_base . 'rawg.php' ) ) $delete_rawx_files[] =  $view_dir . $tracking_id . '/view_pv/raw_g/' . $delete_base . 'rawg.php';
                                    }
// koji maruyama end
                                }
                            }
					...
					$result = $wpdb->get_results(
						...
						$raw_g_dirlist = $this->wrap_dirlist( $raw_g_dir );
						...
						if ( ! empty( $result ) ) {
                            ...
                            if ( $before_yesterday ) {
                                ...
                                for ($i = 0, $raw_g_file_max = $this->wrap_count($raw_g_dirlist); $i < $raw_g_file_max; $i++) {
                                    if ($this->wrap_substr($raw_g_dirlist[$i]['name'], 0, 10) !== $s_date) {
                                        continue;
                                    }
                                    $raw_g_data_ary = $this->wrap_unserialize($this->wrap_get_contents($raw_g_dir . $raw_g_dirlist[$i]['name']));
                                }
                            }
							...
							foreach ($result as $idx => $row ) {
								...
								// raw_x 集約（P7-D PR1 #1334: 行単位集約を collect_rawx_for_row() へ抽出。出力 byte 不変）
								// P7-D PR3 #1379: cutover ON では view_pv ループで raw_x を作らない（独立ステップが本番正本）。
								if ( ! self::PR3_RAWX_CUTOVER_ENABLED ) {
									$rawx = $this->collect_rawx_for_row( $row, $tracking_id, $url_hash, $qa_id, $lp_time, $before_yesterday, $raw_p_data_ary, $raw_c_data_ary, $raw_e_data_ary, $raw_g_data_ary, $wp_filesystem );
									if ( null !== $rawx['raw_p'] ) { $raw_p_ary[] = $rawx['raw_p']; $t_raw_p_ary[$tracking_id][] = $rawx['raw_p']; }
									if ( null !== $rawx['raw_c'] ) { $raw_c_ary[] = $rawx['raw_c']; $t_raw_c_ary[$tracking_id][] = $rawx['raw_c']; }
									if ( null !== $rawx['raw_e'] ) { $raw_e_ary[] = $rawx['raw_e']; $t_raw_e_ary[$tracking_id][] = $rawx['raw_e']; }
									if ( null !== $rawx['raw_g'] ) { $raw_g_ary[] = $rawx['raw_g']; $t_raw_g_ary[$tracking_id][] = $rawx['raw_g']; }
									foreach ( $rawx['delete'] as $rawx_delete_file ) { $delete_rawfiles_ary[] = $rawx_delete_file; }
								}
							}
							...
							// P7-D PR3 #1379: cutover ON では 'all' raw_x split を書かない（独立ステップが本番正本）。view_pv-split は常に書く。
							if ( ! self::PR3_RAWX_CUTOVER_ENABLED ) {
								...
								$this->wrap_put_contents( $raw_g_dir . $filename_base . 'raw_g-split.php', $this->wrap_serialize( $raw_g_ary ) );
							}
							...
							foreach ( $t_newary as $tracking_id => $ary ) {
								...
								if ( ! $wp_filesystem->exists( $view_dir . $tracking_id . '/view_pv/raw_g/' ) ) {
									$wp_filesystem->mkdir( $view_dir . $tracking_id . '/view_pv/raw_g/' );
								}
								// P7-D PR3 #1379: cutover ON では tid 別 raw_x split を書かない（独立ステップが本番正本）。view_pv-split は常に書く。
								if ( ! self::PR3_RAWX_CUTOVER_ENABLED ) {
									...
									$this->wrap_put_contents( $view_dir . $tracking_id . '/view_pv/raw_g/' . $filename_base . 'raw_g-split.php', $this->wrap_serialize( $t_raw_g_ary[$tracking_id] ?? [] ) );
								}
					...
					if( $this->wrap_count( $reqfiles ) == 0 && $this->wrap_count( $proc_reqfiles ) == 0 && $this->wrap_count( $done_reqfiles ) > 0 ){
						...
						foreach ($all_temp_files as $temp_file_ary) {
							...
							if ( self::is_delete_rawfiles_temp( $temp_file_ary['name'] ) ) {
							...
							if ( self::is_verid_update_targets_temp( $temp_file_ary['name'] ) ) {
								// シリアライズされたデータをアンシリアライズ
						...
						//依頼処理結果ファイルを結合
						// P7-D PR3 #1379: cutover ON では raw_x split が存在しないので view_pv のみ merge（二重の保険）。
						$types = self::PR3_RAWX_CUTOVER_ENABLED ? ['view_pv'] : ['view_pv', 'raw_p', 'raw_c', 'raw_e', 'raw_g'];  // 処理するファイルタイプ
						...
						foreach ($types as $type) {
							...
							foreach ( $base_dirs as $base_dir ) {
								...
								foreach ($files as $file) {
									$date = self::find_yyyy_mm_dd_underscore( $file );  // ファイル名から日付を抽出
								...
								foreach ($date_files as $date => $files) {
									...
									usort($files, function($a, $b) {
										$a_id = self::extract_first_pv_id( $a );
										$b_id = self::extract_first_pv_id( $b );
										return $a_id <=> $b_id;
									});
				...
				case 'Night>Make view file>View_pv>Make index loop>Make':
					...
					if ( isset( $ary_tids_for_i[0] ) ) {
						...
						// #1256 Phase 1: reader/source/medium/campaign の4種 index は src 全域に読み手が存在しない（生成のみ）。
						// 最大サイトでは reader index の全ブロックメモリ展開が memory_limit 枯渇の主因だったため、
						// 読み手のある page / version のみ生成する。詳細: Epic #1256
						$indexids = [ 'page', 'version' ];
						foreach ( $indexids as $indexid ) {
							...
							while ( ! $is_endday ) {
								...
								if ( $qahm_time->str_to_unixtime( $e_datetime ) <= $qahm_time->str_to_unixtime( $s_dateend ) ) {
									...
									// #1256 Phase 1: index 再構築のピークメモリを記録（メモリ安全マージンの実測用）
									$qahm_log->info( 'index rebuild done. tid:' . $now_tid . ' indexid:' . $indexid . ' peak_mb:' . round( memory_get_peak_usage( true ) / 1048576 ) );
								} else {
									$is_endday = false;
									$s_datetime = $qahm_time->xday_str( 1, $s_datetime, QAHM_Time::DEFAULT_DATETIME_FORMAT );
								}
				...
				case 'Night>Make view file>View_pv>End':
					...
					// ---next
					// P7-D PR2 #1352 / PR3 #1379: raw_x 独立集約ステップ。PR2=staging 検証 / PR3=本番正本（production）。
					// 既定（両 false）は従来どおり Readers へ直行＝本番フロー完全不変。
					if ( self::PR2_RAWX_STAGE_ENABLED || self::PR3_RAWX_CUTOVER_ENABLED ) {
						$cron_status = 'Night>Make raw_x file>Stage';
					} else {
				...
				case 'Night>Make raw_x file>Stage':
					$this->backup_prev_status( $cron_status );
					// raw_x を qa_pv_log 駆動で集約。$production=true（PR3 cutover）なら本番 raw_X/ へ書き・旧日次削除・-done 削除を担う。
					// $production=false（PR2 のみ）は staging（raw_X_new/）へ二重生成し本番不触＝byte 等価検証用。PR3 が staging より優先。
					$rawx_production = self::PR3_RAWX_CUTOVER_ENABLED;
					$this->stage_rawx_dual_generation( $view_dir, $viewpv_dir, $temp_dir, $raw_p_dir, $raw_c_dir, $raw_e_dir, $raw_g_dir, $del_rawfileslist_file, $rawx_production, $wp_filesystem );
					// ---next
					$cron_status = 'Night>Make view file>Readers>Start';
					$this->set_next_status( $cron_status );
					break;
				...
				case 'Night>Make view file>End':
					...
					// ---next
					$cron_status = 'Night>Make column-db>Start';
					$this->set_next_status( $cron_status );
					break;
				...
				// Making Column-DB
				// ----------
				case 'Night>Make column-db>Start':
					$this->backup_prev_status( $cron_status );
					$columndb_cron = new QAHM_ColumnDB_Cron();
					$cron_status = $columndb_cron->start();
					$this->set_next_status( $cron_status );
					break;

				case 'Night>Make column-db>Loop':
					$this->backup_prev_status( $cron_status );
					$columndb_cron = new QAHM_ColumnDB_Cron();
					$cron_status = $columndb_cron->process_loop();
					$this->set_next_status( $cron_status );
					$while_continue = false; // yield
					break;

				case 'Night>Make column-db>End':
					$this->backup_prev_status( $cron_status );
					$columndb_cron = new QAHM_ColumnDB_Cron();
					$cron_status = $columndb_cron->end();
					$this->set_next_status( $cron_status );
					break;
				...
				case 'Night>Delete>Files>Xmonth ago':
					...
					for ( $iii = 0; $iii < $this->wrap_count( $search_dirs ); $iii++ ) {   // 再帰のためループ毎にcount関数を実行しなければならない
						...
						// 検索対象外のディレクトリ
						// report/ は列DB等の長期の正本を含むため除外（Issue #1474。列ファイルは変換時の一度しか書かれず mtime が更新されない＝除外しないと約3ヶ月で削除される）
						if ( false !== $this->wrap_strpos( $dir, 'qa-zero-data/view/' ) ||
						     false !== $this->wrap_strpos( $dir, 'qa-zero-data/crawler/' ) ||
						     false !== $this->wrap_strpos( $dir, 'qa-zero-data/report/' ) ) {
							continue;
						}
...
// delete same_month -1
							...
							foreach ( $allfiles as $file ) {
								...
								if ( is_file( $now_viewpv_dir . $filename ) ) {
									...
									if ( $qahm_time->str_to_unixtime( $s_datetime ) <= $qahm_time->str_to_unixtime( $f_datetime ) ) {
										...
										$session_cnt = 0;
										$all_readers = [];
										// #1287 P2-α: days_access は ColumnDB（allpv）読みを正とする（pv / reader_id の2列のみ）。
										// view_pv は夜間チェーン内で qa_pv_log と行数突合済みのため、行数一致を allpv 採用の
										// 条件にする（ドリフト日の自己修復を温存）。完了マーカー等の保証が不成立の日も view_pv を使う。
										$allpv_cols = is_object( $qahm_db ) ? $qahm_db->load_allpv_columns( $now_tid, str_replace( '-', '', $f_date ), array( 'pv', 'reader_id' ) ) : null;
										if ( null !== $allpv_cols && $this->wrap_count( $allpv_cols['pv'] ) === $pv_cnt ) {
											foreach ( $allpv_cols['pv'] as $allpv_row => $allpv_pv_no ) {
												//count session 当日の1ページ目（LP着地）をカウント
												if ( 1 === (int) $allpv_pv_no ) {
													++$session_cnt;
													$all_readers[] = (int) $allpv_cols['reader_id'][ $allpv_row ];
												}
											}
										} else {
				...
				case 'Night>SC get>Each day>Start':
					...
					if ( $timelimit_forced_end ) {
						...
						$this->wrap_put_contents( $target_memo_file, $this->wrap_serialize( $gsc_target_memo ) );
						$qahm_log->debug( 'GSC get loop timed out. ' . $now_search_cnt . ' / ' . $max_search_cnt );
						...
						$gsc_target_memo['sites'][$processing_site_idx]['loop_memo'] = $loop_memo;
						$this->wrap_put_contents( $target_memo_file, $this->wrap_serialize( $gsc_target_memo ) );
						$qahm_log->debug( 'GSC get skipped loop. ' . ( $now_search_cnt + 1 ) . ' / ' . $max_search_cnt );
				...
				case 'Night>SC get>End':
					...
					// ---next
					$cron_status = 'Night>Make column-db gsc>Start';
					$this->set_next_status( $cron_status );
					break;


				// ----------
				// Making column-db gsc (GSC列DB変換)
				// ----------
				case 'Night>Make column-db gsc>Start':
					$this->backup_prev_status( $cron_status );
					$columndb_gsc_cron = new QAHM_ColumnDB_GscCron();
					$cron_status = $columndb_gsc_cron->start();
					$this->set_next_status( $cron_status );
					break;

				case 'Night>Make column-db gsc>Loop':
					$this->backup_prev_status( $cron_status );
					$columndb_gsc_cron = new QAHM_ColumnDB_GscCron();
					$cron_status = $columndb_gsc_cron->process_loop();
					$this->set_next_status( $cron_status );
					$while_continue = false; // yield
					break;

				case 'Night>Make column-db gsc>End':
					$this->backup_prev_status( $cron_status );
					$columndb_gsc_cron = new QAHM_ColumnDB_GscCron();
					$cron_status = $columndb_gsc_cron->end();
					// GA4ステージを挿入（GSC End -> GA4 get Start）
					$cron_status = 'Night>GA4 get>Start';
					$this->set_next_status( $cron_status );
					break;


				// ----------
				// GA4 get (GA4 Data API 月次レポート取得)
				// ----------
				case 'Night>GA4 get>Start':
					$this->backup_prev_status( $cron_status );

					global $qahm_data_api;

					// GA4プロパティが設定されているサイトについて、
					// 過去 QAHM_GA4_BACKFILL_MONTHS ヶ月の未取得月を全てtargetsに積む（T60）。
					// 毎晩Startで check_file ベースに再構築する（状態分岐なし）。
					$ga4_settings = get_option( 'qahm_ga4_settings', array() );
					$siteary = $qahm_data_api->get_sitemanage();
					$ga4_targets = array();

					$ga4_backfill_months = defined( 'QAHM_GA4_BACKFILL_MONTHS' ) ? (int) QAHM_GA4_BACKFILL_MONTHS : 13;
					if ( $ga4_backfill_months < 1 ) {
						$ga4_backfill_months = 1;
					}

					if ( ! empty( $siteary ) && ! empty( $ga4_settings ) ) {
						foreach ( $siteary as $site ) {
							$tid = $site['tracking_id'];
							if ( ! isset( $ga4_settings[ $tid ] ) || empty( $ga4_settings[ $tid ]['ga4_property_id'] ) ) {
								continue;
							}
							$ga4_dir = $this->get_data_dir_path( 'view/' . $tid . '/ga4' );

							// n=0 (当月) から backfill_months まで順に遡る（T66）。
							// 月初日(Y-m-01)を起点に減算すること。`strtotime('-N month')` を
							// 現在日基準で呼ぶと、3/31 → 2/31(存在しない) → 3/3 と正規化され
							// 結果月が現在月に戻ってしまい、月が丸ごとスキップされて重複targetを生む。
							$ga4_month_base = gmdate( 'Y-m-01' );
							for ( $n = 0; $n <= $ga4_backfill_months; $n++ ) {
								$year_month = gmdate( 'Y-m', strtotime( $ga4_month_base . ' -' . $n . ' month' ) );
								$check_file = $ga4_dir . $year_month . '_ga4_age_gender.php';
								// 当月(n=0)はデータが日々増えるため毎晩上書き。過去月は既存ファイルをスキップ。
								if ( $n > 0 && $this->wrap_exists( $check_file ) ) {
									continue;
								}
								$ga4_targets[] = array(
									'tracking_id' => $tid,
									'url'         => $site['url'],
									'property_id' => $ga4_settings[ $tid ]['ga4_property_id'],
					...
					if ( empty( $ga4_targets ) ) {
						$cron_status = 'Night>GA4 get>End';
						$this->set_next_status( $cron_status );
						break;
					}
					...
					$this->wrap_put_contents( $temp_dir . 'cron_ga4_get_memo.php', $this->wrap_serialize( $ga4_get_memo ) );

					$cron_status = 'Night>GA4 get>Each day>Start';
					$this->set_next_status( $cron_status );
					break;

				case 'Night>GA4 get>Each day>Start':
					$this->backup_prev_status( $cron_status );

					$ga4_memo_file = $temp_dir . 'cron_ga4_get_memo.php';
					if ( ! $this->wrap_exists( $ga4_memo_file ) ) {
						$cron_status = 'Night>GA4 get>End';
						$this->set_next_status( $cron_status );
						break;
					}

					$ga4_get_memo = $this->wrap_unserialize( $this->wrap_get_contents( $ga4_memo_file ) );
					$ga4_idx = $ga4_get_memo['index'];

					if ( $ga4_idx >= count( $ga4_get_memo['targets'] ) ) {
						$cron_status = 'Night>GA4 get>End';
						$this->set_next_status( $cron_status );
						break;
					}

					$ga4_target = $ga4_get_memo['targets'][ $ga4_idx ];
					$tid         = $ga4_target['tracking_id'];
					$url         = $ga4_target['url'];
					$property_id = $ga4_target['property_id'];
					$year_month  = $ga4_target['year_month'];

					// Google API初期化
					$qahm_google_api->set_tracking_id( $tid, $url );
					$ga4_scope = array(
						'https://www.googleapis.com/auth/webmasters.readonly',
						'https://www.googleapis.com/auth/analytics.readonly',
					);
					$is_init = $qahm_google_api->init_for_background( 'Google API Integration', $ga4_scope );

					if ( ! $is_init ) {
						// 認証失敗 = 設定/トークンの問題。
						// T60以前は End にジャンプしていたが、backfillで targets が
						// 「サイトA × 13月 + サイトB × 13月 …」と長くなったため、
						// 1サイトの失敗で全サイトが止まってしまう。GSC側（Night>SC get）
						// と同じく「同サイトの残月のみスキップして次サイトへ継続」に統一。
						// check_file が作られないので翌夜の Start で自動再登録される。
						$skip_tid = $tid;
						$skipped_cnt = 0;
						while (
							$ga4_idx < count( $ga4_get_memo['targets'] )
							&& $ga4_get_memo['targets'][ $ga4_idx ]['tracking_id'] === $skip_tid
						) {
							$ga4_idx++;
							$skipped_cnt++;
						}
						$ga4_get_memo['index'] = $ga4_idx;
						$this->wrap_put_contents( $ga4_memo_file, $this->wrap_serialize( $ga4_get_memo ) );
						$qahm_log->warning( 'GA4 get: Authentication failed for ' . $skip_tid . ' — skipped ' . $skipped_cnt . ' target(s) for this site; continuing with next site' );
						$cron_status = 'Night>GA4 get>Each day>Start';
						$while_continue = false;
						$this->set_next_status( $cron_status );
						break;
					...
					// GA4月次レポート取得
					$reports = $qahm_google_api->fetch_ga4_monthly_reports( $property_id, $year_month );

					if ( isset( $reports['error'] ) ) {
						$qahm_log->warning( 'GA4 get: API error for ' . $tid . ' — ' . json_encode( $reports['error'] ) );
						$ga4_get_memo['index'] = $ga4_idx + 1;
						$this->wrap_put_contents( $ga4_memo_file, $this->wrap_serialize( $ga4_get_memo ) );
						$cron_status = 'Night>GA4 get>Each day>Start';
						$while_continue = false;
						$this->set_next_status( $cron_status );
					...
					// 中間ファイル保存
					$ga4_dir = $this->get_data_dir_path( 'view/' . $tid . '/ga4' );
					if ( ! $this->wrap_exists( $ga4_dir ) ) {
						$this->wrap_mkdir( $ga4_dir );
					}

					$report_types = array( 'age_gender', 'country', 'region' );
					foreach ( $report_types as $type ) {
						if ( isset( $reports[ $type ] ) ) {
							$file_path = $ga4_dir . $year_month . '_ga4_' . $type . '.php';
							$this->wrap_put_contents( $file_path, $this->wrap_serialize( $reports[ $type ] ) );
						}
					}

					$qahm_log->debug( 'GA4 get: Saved reports for ' . $tid . ' / ' . $year_month );
				...
				case 'Night>GA4 get>End':
					$this->wrap_delete( $temp_dir . 'cron_ga4_get_memo.php' );
					$cron_status = 'Night>Make column-db ga4>Start';
					$this->set_next_status( $cron_status );
					break;


				// ----------
				// Making column-db ga4 (GA4列DB変換)
				// ----------
				case 'Night>Make column-db ga4>Start':
					$this->backup_prev_status( $cron_status );
					$columndb_ga4_cron = new QAHM_ColumnDB_Ga4Cron();
					$cron_status = $columndb_ga4_cron->start();
					$this->set_next_status( $cron_status );
					break;

				case 'Night>Make column-db ga4>Loop':
					$this->backup_prev_status( $cron_status );
					$columndb_ga4_cron = new QAHM_ColumnDB_Ga4Cron();
					$cron_status = $columndb_ga4_cron->process_loop();
					$this->set_next_status( $cron_status );
					$while_continue = false;
					break;

				case 'Night>Make column-db ga4>End':
					$this->backup_prev_status( $cron_status );
					$columndb_ga4_cron = new QAHM_ColumnDB_Ga4Cron();
					$cron_status = $columndb_ga4_cron->end();
					$this->set_next_status( $cron_status );
					break;
				...
				case 'Night>Make Goal file>making loop>Start':
					...
					//}
						...
						$site_tracking_ids = $this->wrap_array_column( $sitemanage, 'tracking_id' );
								...
								if ( ! $log_ary[ $each_month['Y-m_str'] ]['done'] ) {
											...
											if ( $okutime < $file_mtime ) {
												switch ( $goal_ary['gtype'] ) {
													...
													case 'gtype_event' :
														...
														$gfile_complete = true;
													break;
													// #1345: gtype_dlevent は「ver2.2 以前の旧 goal ファイル」が存在し得ない
													// 新形式のため、ここでの完成判定の対象外。$make_goal_file = true のまま
													// 通常の生成経路へ委ねる（ここで完成扱いにすると未生成のまま確定してしまう）。
													// ★ 何もしない case をあえて置いているのは、次に gtype を足す人が
													//   「漏れている」と誤解して gtype_page 側へ足すのを防ぐため。
													case 'gtype_dlevent' :
													break;
													case 'gtype_click' :																												
														...
														if ( ! is_null( $goal_comp_sessions ) ) {
															...
															$qahm_log->debug( 'extract_click_goal_comp_sessions returned null. tracking_id: ' . $tracking_id . ', gid: ' . $gid . ', from_date: ' . $each_month['a_date'] . ', to_date: ' . $each_month['b_date'] );
														}													
													break;
												}
											}
									...
									if ( $make_goal_file ) {
										...
										if ( ! is_null( $goal_comp_sessions ) ) {
											...
											$qahm_log->debug( 'fetch_goal_comp_sessions returned null. tracking_id: ' . $tracking_id . ', gid: ' . $gid . ', from_date: ' . $from_date . ', to_date: ' . $to_date );
										}
												...
												switch ( $goal_ary['gtype'] ) {
													...
													case 'gtype_click' :																								
														...
														if ( ! is_null( $goal_comp_sessions ) ) {
															...
															$qahm_log->debug( 'extract_click_goal_comp_sessions returned null. tracking_id: ' . $tracking_id . ', gid: ' . $gid . ', from_date: ' . $from_date . ', to_date: ' . $to_date );
														}
													break;
												}
											}
	...
	// ============================================================
	// String parsing helpers (T85-C-fix-3: preg_match -> strpos/substr)
	// ============================================================

	private static function is_all_digits( $s ) {
		return $s !== '' && ctype_digit( $s );
	}

	/**
	 * Match `^(.*)_(\d+)\.php$` against $name.
	 * Returns [base_name, digits] on success, null on failure.
	 */
	private static function parse_session_filename( $name ) {
		...
		$digits = substr( $stem, $u + 1 );
		if ( ! self::is_all_digits( $digits ) ) {
			return null;
		}
		return array( substr( $stem, 0, $u ), $digits );
	}

	/**
	 * Find first `_YYYY-MM-DD_` segment and return `YYYY-MM-DD`, or '' if none.
	 * Equivalent to preg_match('/_(\d{4}-\d{2}-\d{2})_/', $name, $m).
	 */
	private static function find_yyyy_mm_dd_segment( $name ) {
		$len = strlen( $name );
		$pos = strpos( $name, '_' );
		while ( $pos !== false ) {
			if ( $pos + 11 < $len
				...
				$year  = substr( $name, $pos + 1, 4 );
				$month = substr( $name, $pos + 6, 2 );
				$day   = substr( $name, $pos + 9, 2 );
				if ( ctype_digit( $year ) && ctype_digit( $month ) && ctype_digit( $day ) ) {
					return $year . '-' . $month . '-' . $day;
				}
			}
			$pos = strpos( $name, '_', $pos + 1 );
		}
		return '';
	}

	/**
	 * Match trailing `_<digits>.php` and return digits (may be ''), null if no `.php`.
	 *
	 * Original regex `/_([0-9]*).php/` left `.` unescaped (matches any single char before `php`).
	 * In practice cron only generates `.php` files, so this stricter `.php`-only matcher is
	 * equivalent for real inputs but not literally regex-equivalent. Acceptable trade-off.
	 */
	private static function parse_trailing_session_no( $filename ) {
	...
	/**
	 * Match `^viewpv_proc_request-(\d+-\d+-\d+)-\d+\.php$`.
	 * Returns the captured `YYYY-MM-DD` (or `n1-n2-n3`), null on failure.
	 */
	private static function parse_viewpv_proc_request( $filename ) {
		$prefix     = 'viewpv_proc_request-';
		...
		$middle = substr( $filename, $prefix_len, -4 );
		$parts  = explode( '-', $middle );
		if ( count( $parts ) !== 4 ) {
			return null;
		}
		foreach ( $parts as $p ) {
			if ( ! self::is_all_digits( $p ) ) {
				return null;
			}
		}
	...
	/**
	 * Match `^processing_by-(\d+)-viewpv_proc_request-\d+-\d+-\d+-\d+\.php$`.
	 * Returns the captured pid, null on failure.
	 */
	private static function parse_processing_by( $filename ) {
		$p1     = 'processing_by-';
		$p1_len = strlen( $p1 );
		if ( strncmp( $filename, $p1, $p1_len ) !== 0 ) {
			return null;
		}
		$marker     = '-viewpv_proc_request-';
		$marker_pos = strpos( $filename, $marker, $p1_len );
		if ( $marker_pos === false ) {
			return null;
		}
		$pid = substr( $filename, $p1_len, $marker_pos - $p1_len );
		if ( ! self::is_all_digits( $pid ) ) {
			return null;
		}
		$rest = substr( $filename, $marker_pos + strlen( $marker ) );
		if ( substr( $rest, -4 ) !== '.php' ) {
			return null;
		}
		$nums  = substr( $rest, 0, -4 );
		$parts = explode( '-', $nums );
		if ( count( $parts ) !== 4 ) {
			return null;
		}
	...
	/**
	 * Match `^done_by-\d+-viewpv_proc_request-\d+-\d+-\d+-\d+\.php$` (bool only).
	 */
	private static function is_done_by_format( $filename ) {
		$p1     = 'done_by-';
		$p1_len = strlen( $p1 );
		if ( strncmp( $filename, $p1, $p1_len ) !== 0 ) {
			return false;
	...
	/**
	 * Match `^view_pv_process-(\d+)\.php$`. Returns pid, null on failure.
	 */
	private static function parse_view_pv_process( $filename ) {
		$prefix     = 'view_pv_process-';
		$prefix_len = strlen( $prefix );
		if ( strncmp( $filename, $prefix, $prefix_len ) !== 0 ) {
			return null;
		}
		...
		$pid = substr( $filename, $prefix_len, -4 );
		if ( ! self::is_all_digits( $pid ) ) {
			return null;
		}
		return $pid;
	...
	/**
	 * Strip leading `processing_by-<digits>-` from $name. If not matched, return $name as-is
	 * (mirrors preg_replace default behavior).
	 */
	private static function strip_processing_by( $name ) {
		$prefix     = 'processing_by-';
		$prefix_len = strlen( $prefix );
		if ( strncmp( $name, $prefix, $prefix_len ) !== 0 ) {
			return $name;
		}
		$rest = substr( $name, $prefix_len );
		$dash = strpos( $rest, '-' );
		if ( $dash === false ) {
			return $name;
		}
		$pid = substr( $rest, 0, $dash );
		if ( ! self::is_all_digits( $pid ) ) {
			return $name;
		}
		return substr( $rest, $dash + 1 );
	}

	/**
	 * Match `^delete_rawfiles_ary_temp-\d+.php$`.
	 */
	private static function is_delete_rawfiles_temp( $name ) {
		$prefix     = 'delete_rawfiles_ary_temp-';
		$prefix_len = strlen( $prefix );
		if ( strncmp( $name, $prefix, $prefix_len ) !== 0 ) {
			return false;
		}
		...
		$digits = substr( $name, $prefix_len, -4 );
		return self::is_all_digits( $digits );
	}

	/**
	 * Match `^verid_update_targets_ary_temp-\d+.php$`.
	 */
	private static function is_verid_update_targets_temp( $name ) {
		$prefix     = 'verid_update_targets_ary_temp-';
		$prefix_len = strlen( $prefix );
		if ( strncmp( $name, $prefix, $prefix_len ) !== 0 ) {
			return false;
		}
	...
	/**
	 * Find first occurrence of `YYYY-MM-DD_` (no leading underscore required) and return
	 * the date, or '' if not found. Equivalent to preg_match('/(\d{4}-\d{2}-\d{2})_/').
	 */
	private static function find_yyyy_mm_dd_underscore( $s ) {
		$len = strlen( $s );
		for ( $i = 0; $i + 11 <= $len; $i++ ) {
			if ( $s[ $i + 4 ] === '-'
				&& $s[ $i + 7 ] === '-'
	...
	/**
	 * Extract the first `_<digits>-<digits>_` group's leading digits as int, 0 if not found.
	 * Equivalent to preg_match('/_(\d+)-\d+_/', $name, $m); (int)$m[1].
	 */
	private static function extract_first_pv_id( $name ) {
		$len = strlen( $name );
		$pos = strpos( $name, '_' );
		while ( $pos !== false ) {
			$i             = $pos + 1;
			...
			if ( $i > $digits_start && $i < $len && $name[ $i ] === '-' ) {
				$first_digits = substr( $name, $digits_start, $i - $digits_start );
				$j            = $i + 1;
				$second_start = $j;
				while ( $j < $len && ctype_digit( $name[ $j ] ) ) {
					$j++;
				}
				if ( $j > $second_start && $j < $len && $name[ $j ] === '_' ) {
	...
	/**
	 * P7-D PR1 #1334: raw_x（raw_p/c/e/g）の行単位集約を抽出。
	 * 旧インライン（view_pv ループ内）と出力 byte 不変。呼び出し側が返り値を
	 * $raw_*_ary / $t_raw_*_ary[$tracking_id] / $delete_rawfiles_ary へ同順 append する。
	 * p/c/e は is_raw_* で（空でも）採用、raw_g は !empty のときのみ採用。
	 */
	private function collect_rawx_for_row( $row, $tracking_id, $url_hash, $qa_id, $lp_time, $before_yesterday, $raw_p_data_ary, $raw_c_data_ary, $raw_e_data_ary, $raw_g_data_ary, $wp_filesystem ) {
		$ret = array( 'raw_p' => null, 'raw_c' => null, 'raw_e' => null, 'raw_g' => null, 'delete' => array() );
		...
		if ( $row->is_raw_p ) {
			...
			if ( $wp_filesystem->exists( $file_p ) ) {
				...
				foreach ( $raw_p_data_ary as $raw_p_data ) {
					if ( (int) $pv_id !== (int) $raw_p_data['pv_id'] ) {
						continue;
					}
			...
			$ret['raw_p']   = array( 'pv_id' => $pv_id, 'raw_p' => $p_str );
			$ret['delete'][] = $file_p;
		}
		if ( $row->is_raw_c ) {
			$c_str  = '';
			...
			if ( $wp_filesystem->exists( $file_c ) ) {
				...
				foreach ( $raw_c_data_ary as $raw_c_data ) {
					if ( (int) $pv_id !== (int) $raw_c_data['pv_id'] ) {
						continue;
					}
			...
			$ret['raw_c']   = array( 'pv_id' => $pv_id, 'raw_c' => $c_str );
			$ret['delete'][] = $file_c;
		}
		if ( $row->is_raw_e ) {
			$e_str  = '';
			...
			if ( $wp_filesystem->exists( $file_e ) ) {
				...
				foreach ( $raw_e_data_ary as $raw_e_data ) {
					if ( (int) $pv_id !== (int) $raw_e_data['pv_id'] ) {
						continue;
					}
			...
			$ret['raw_e']   = array( 'pv_id' => $pv_id, 'raw_e' => $e_str );
			$ret['delete'][] = $file_e;
		}

		// raw_g: is_raw_g フラグがないため、ファイル存在チェックで判定
		$g_str  = '';
		$file_g = $raw_dir . $raw_name . '-g' . '.php';
		if ( $wp_filesystem->exists( $file_g ) ) {
			$g_str = $this->wrap_get_contents( $file_g );
		} elseif ( $before_yesterday ) {
			foreach ( $raw_g_data_ary as $raw_g_data ) {
				if ( (int) $pv_id !== (int) $raw_g_data['pv_id'] ) {
					continue;
				}
				$g_str = $raw_g_data['raw_g'];
				break;
			}
		}
		if ( ! empty( $g_str ) ) {
			$ret['raw_g']   = array( 'pv_id' => $pv_id, 'raw_g' => $g_str );
			$ret['delete'][] = $file_g;
		}
	...
	/**
	 * P7-D PR2 #1352 / PR3 #1379: raw_x 日次集約の独立経路。
	 *
	 * view_pv ビルドとは独立に、当 run で再生成された日（temp/viewpv_create_hist.php）の raw_x を
	 * qa_pv_log 駆動で集約する。
	 * - $production=false（PR2 staging 検証）: staging（raw_{p,c,e,g}_new/）へ出力。本番 raw_X/ 不触・-done 削除なし。
	 * - $production=true（PR3 cutover）: 各対象日で「既存本番日次を source に読む→集約→その日の本番日次を日付 glob で
	 *   全削除→新名で本番 raw_X/ へ書く」を自己完結で行い、view_pv から外れた raw_x の本番正本となる。読んだ個別
	 *   -done は $del_rawfileslist_file（2日ローテーション）へ append し既存の遅延削除機構に乗せる。
	 *
	 * @param string $view_dir   data/view/.
	 * @param string $viewpv_dir 'all' の view_pv/ ディレクトリ.
	 * @param string $temp_dir   data/temp/.
	 * @param string $raw_p_dir  'all' raw_p/.
	 * @param string $raw_c_dir  'all' raw_c/.
	 * @param string $raw_e_dir  'all' raw_e/.
	 * @param string $raw_g_dir  'all' raw_g/.
	 * @param string $del_rawfileslist_file 個別 -done 削除リスト（ローテーション本体）。production 時のみ append。
	 * @param bool   $production true=本番正本（PR3 cutover）/ false=staging 検証（PR2）.
	 * @param object $wp_filesystem WP_Filesystem.
	 * @return void
	 */
	private function stage_rawx_dual_generation( $view_dir, $viewpv_dir, $temp_dir, $raw_p_dir, $raw_c_dir, $raw_e_dir, $raw_g_dir, $del_rawfileslist_file, $production, $wp_filesystem ) {
		global $qahm_log;

		// 当 run で再生成された日のリスト（旧 View_pv が再生成する日＝byte 等価の対象日）.
		$hist_file = $temp_dir . 'viewpv_create_hist.php';
		if ( ! $wp_filesystem->exists( $hist_file ) ) {
			return;
		}
		$hist = $this->wrap_unserialize( $this->wrap_get_contents( $hist_file ) );
		if ( ! is_array( $hist ) || empty( $hist ) ) {
			return;
		}
		$target_dates = array_keys( $hist );

		if ( ! $production ) {
			// PR2 staging 検証: staging（*_new/）の残骸を冪等掃除（staging 内限定・本番 raw_X/ は対象外）.
			// 掃除対象 tid は sitemanage でなく **ディスク実在の view/ 配下**から列挙する（255 削除済みでも当日 pv_log が
			// 残る tid の staging を取りこぼさない＝孤児ファイルによる偽 DIFF を防ぐ。コードレビュー🟡#1 反映）.
			$this->stage_rawx_cleanup_staging( $view_dir, $wp_filesystem );
		}

		// production 時、読んだ個別 -done をまとめて削除リストへ積む（2日ローテーションに乗せる）.
		$delete_done_files = array();
		foreach ( $target_dates as $s_date ) {
			$deletes = $this->stage_rawx_for_date( (string) $s_date, $view_dir, $viewpv_dir, $raw_p_dir, $raw_c_dir, $raw_e_dir, $raw_g_dir, $production, $wp_filesystem );
			if ( $production && is_array( $deletes ) && ! empty( $deletes ) ) {
				$delete_done_files = array_merge( $delete_done_files, $deletes );
			}
		}

		if ( $production && ! empty( $delete_done_files ) ) {
			$this->stage_rawx_append_delete_list( $del_rawfileslist_file, $delete_done_files, $wp_filesystem );
		}

		if ( isset( $qahm_log ) ) {
			$qahm_log->info( 'P7-D raw_x ' . ( $production ? 'cutover(production)' : 'stage(staging)' ) . ' done. dates:' . implode( ',', $target_dates ) );
		}
	}

	/**
	 * staging（raw_{p,c,e,g}_new/）の中身を掃除する（'all' ＋ 各 tid）。本番 raw_X/ には触れない。
	 *
	 * 掃除対象 tid は **ディスク実在の view/ 配下ディレクトリ**から列挙する（'all' もそのうちの1つ）。
	 * sitemanage に依存しないため、status 255 で除外された tid でも当日 pv_log が残れば staging を取りこぼさない。
	 *
	 * @param string $view_dir   data/view/.
	 * @param object $wp_filesystem WP_Filesystem.
	 * @return void
	 */
	private function stage_rawx_cleanup_staging( $view_dir, $wp_filesystem ) {
		$types    = array( 'raw_p', 'raw_c', 'raw_e', 'raw_g' );
		$tid_dirs = $this->wrap_dirlist( $view_dir );
		if ( ! $tid_dirs ) {
			return;
		}
		foreach ( $tid_dirs as $tid_entry ) {
			$viewpv = $view_dir . $tid_entry['name'] . '/view_pv/';
			foreach ( $types as $type ) {
				$dir = $viewpv . $type . '_new/';
	...
	/**
	 * 指定日の raw_x を qa_pv_log 駆動で集約し staging へ出力する（旧 merge と byte 一致する形）。
	 *
	 * 旧経路との byte 一致の要点:
	 * - 行順 = pv_id 昇順（SELECT で ORDER BY pv_id。旧は merge の split 連結結果が pv_id 単調＝dev5 実測済）。
	 * - 配列構造 = [0..N-1 => ['pv_id'=>, 'raw_X'=>]]（collect_rawx_for_row の戻り＋[] append。旧は array_merge で 0..N-1 再採番）。
	 * - 直列化 = wrap_serialize（igbinary）。
	 * - ファイル名 pv_id レンジ = 全日 min/max（= 旧 merge の 'all' レンジ。全 raw_x が 'all' レンジで命名されるクォーク L4119-4125 を再現）。tid 別も同レンジ。
	 *
	 * @param string $s_date     対象日 'YYYY-MM-DD'.
	 * @param string $view_dir   data/view/.
	 * @param string $viewpv_dir 'all' view_pv/.
	 * @param string $raw_p_dir  'all' raw_p/.
	 * @param string $raw_c_dir  'all' raw_c/.
	 * @param string $raw_e_dir  'all' raw_e/.
	 * @param string $raw_g_dir  'all' raw_g/.
	 * @param bool   $production true=本番 raw_X/ へ書き旧日次を削除（PR3 cutover）/ false=staging（PR2）.
	 * @param object $wp_filesystem WP_Filesystem.
	 * @return array production 時に読んだ個別 -done ファイルパス（削除リスト用）。staging 時は空配列.
	 */
	private function stage_rawx_for_date( $s_date, $view_dir, $viewpv_dir, $raw_p_dir, $raw_c_dir, $raw_e_dir, $raw_g_dir, $production, $wp_filesystem ) {
		global $wpdb, $qahm_db, $qahm_time;
		...
		// before_yesterday 判定（旧 L3612-3618 と同値）.
		$max_datetime     = $qahm_time->xday_str( -1 ) . ' 23:59:59';
		$before_yesterday = true;
		if ( $qahm_time->str_to_unixtime( $max_datetime ) <= $qahm_time->str_to_unixtime( $s_dateend ) ) {
			$before_yesterday = false;
		}

		// 当日 qa_pv_log を pv_id 昇順で取得（旧 L3440-3447 と同 WHERE ＋ ORDER BY pv_id）.
		$table_name = $wpdb->prefix . 'qa_pv_log';
		// phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared -- Verified inline $wpdb->prepare; identifiers fixed.
		$result = $wpdb->get_results(
			$wpdb->prepare(
				"SELECT pv_id, reader_id, page_id, device_id, source_id, medium_id, campaign_id, session_no, access_time, pv, speed_msec, browse_sec, is_last, is_newuser, is_raw_p, is_raw_c, is_raw_e
				FROM `{$table_name}`
		...
		if ( empty( $result ) ) {
			// production 時、rebuild 対象日が空（pv_log 0行）になったら、旧経路（C で旧日次削除→merge で空作成）と
			// 同様にその日の本番日次を一掃する（stale 日次の残留＝過大計上を防ぐ。セルフレビュー🟡-1 反映）.
			if ( $production ) {
				$this->stage_rawx_delete_daily_for_date( $view_dir, $s_date, $wp_filesystem );
			}
			return $delete_done;
		}

		// 全日レンジ（= 'all' レンジ）。ORDER BY pv_id ゆえ先頭=最小・末尾=最大.
		$first_pv_id   = $result[0]->pv_id;
		$last_pv_id    = $result[ $this->wrap_count( $result ) - 1 ]->pv_id;
		$filename_base = $s_date . '_' . (string) $first_pv_id . '-' . (string) $last_pv_id . '_';

		// qa_readers（qa_id）/ qa_pages（url_hash, tracking_id）を一括取得（view_pv の重い JOIN はしない）.
		$reader_ids = $qahm_db->get_unique_ids( 'reader_id', $result );
		$page_ids   = $qahm_db->get_unique_ids( 'page_id', $result );
		...
		$pages_data   = array();
		if ( ! empty( $reader_ids ) ) {
			$reader_ph = $qahm_db->create_in_placeholders( $reader_ids );
			$table_r   = $wpdb->prefix . 'qa_readers';
			$query_r   = "SELECT reader_id, qa_id FROM $table_r WHERE reader_id IN ($reader_ph)";
			// phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared -- Verified inline $wpdb->prepare; placeholders match; identifiers fixed.
			$readers_raw = $wpdb->get_results( $wpdb->prepare( $query_r, ...$reader_ids ) );
			foreach ( $readers_raw as $r ) {
				$readers_data[ $r->reader_id ] = $r;
			}
		}
		if ( ! empty( $page_ids ) ) {
			$page_ph = $qahm_db->create_in_placeholders( $page_ids );
			$table_p = $wpdb->prefix . 'qa_pages';
			$query_p = "SELECT page_id, tracking_id, url_hash FROM $table_p WHERE page_id IN ($page_ph)";
			// phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared -- Verified inline $wpdb->prepare; placeholders match; identifiers fixed.
			$pages_raw = $wpdb->get_results( $wpdb->prepare( $query_p, ...$page_ids ) );
			foreach ( $pages_raw as $p ) {
				$pages_data[ $p->page_id ] = $p;
			}
		}

		// before_yesterday は既存 'all' 日次 raw_x から過去日データを供給（旧 L3702-3729 と同じ・read-only）.
		$raw_p_data_ary = array();
		$raw_c_data_ary = array();
		$raw_e_data_ary = array();
		$raw_g_data_ary = array();
		if ( $before_yesterday ) {
			$raw_p_data_ary = $this->stage_rawx_load_daily( $raw_p_dir, $s_date, $wp_filesystem );
			$raw_c_data_ary = $this->stage_rawx_load_daily( $raw_c_dir, $s_date, $wp_filesystem );
			$raw_e_data_ary = $this->stage_rawx_load_daily( $raw_e_dir, $s_date, $wp_filesystem );
			$raw_g_data_ary = $this->stage_rawx_load_daily( $raw_g_dir, $s_date, $wp_filesystem );
		}

		// 集約（'all' ＋ tid 別）。pv_id 昇順 SELECT ゆえ append 順がそのまま昇順.
		$all = array( 'raw_p' => array(), 'raw_c' => array(), 'raw_e' => array(), 'raw_g' => array() );
		$t   = array( 'raw_p' => array(), 'raw_c' => array(), 'raw_e' => array(), 'raw_g' => array() );
		foreach ( $result as $row ) {
			$qa_id       = isset( $readers_data[ $row->reader_id ] ) ? $readers_data[ $row->reader_id ]->qa_id : '';
			$tracking_id = isset( $pages_data[ $row->page_id ] ) ? $pages_data[ $row->page_id ]->tracking_id : '';
			$url_hash    = isset( $pages_data[ $row->page_id ] ) ? $pages_data[ $row->page_id ]->url_hash : '';
			...
			$rawx = $this->collect_rawx_for_row( $row, $tracking_id, $url_hash, $qa_id, $lp_time, $before_yesterday, $raw_p_data_ary, $raw_c_data_ary, $raw_e_data_ary, $raw_g_data_ary, $wp_filesystem );
			foreach ( array( 'raw_p', 'raw_c', 'raw_e', 'raw_g' ) as $type ) {
				if ( null !== $rawx[ $type ] ) {
					$all[ $type ][] = $rawx[ $type ];
					$t[ $type ][ $tracking_id ][] = $rawx[ $type ];
				}
			}
			// production 時のみ、読んだ個別 -done を削除リストへ（staging 時は旧 View_pv 経路が削除責任を持つので触らない）.
			if ( $production && ! empty( $rawx['delete'] ) ) {
				foreach ( $rawx['delete'] as $del_file ) {
					$delete_done[] = $del_file;
				}
			}
		}

		$suffix_map = array( 'raw_p' => 'rawp', 'raw_c' => 'rawc', 'raw_e' => 'rawe', 'raw_g' => 'rawg' );

		if ( $production ) {
			// 本番正本（PR3 cutover）: その日の本番日次を日付 glob で全削除（'all' ＋ 全 tid・旧名/別レンジ名も一掃）
			// → 新名で本番 raw_X/ へ書く。source（既存日次）の read は上で完了済みなので self-clobber しない。
			$this->stage_rawx_delete_daily_for_date( $view_dir, $s_date, $wp_filesystem );

			// 'all' を本番 raw_X/ へ.
			$prod_all_dirs = array( 'raw_p' => $raw_p_dir, 'raw_c' => $raw_c_dir, 'raw_e' => $raw_e_dir, 'raw_g' => $raw_g_dir );
			foreach ( $suffix_map as $type => $suf ) {
				$this->stage_rawx_write( $prod_all_dirs[ $type ], $filename_base . $suf . '.php', $all[ $type ], $wp_filesystem );
			}
			// tid 別を本番 raw_X/ へ（レンジは 'all' の filename_base を流用＝旧クォーク再現）.
			foreach ( $suffix_map as $type => $suf ) {
				foreach ( $t[ $type ] as $tracking_id => $arr ) {
					$prod_dir = $view_dir . $tracking_id . '/view_pv/' . $type . '/';
					$this->stage_rawx_write( $prod_dir, $filename_base . $suf . '.php', $arr, $wp_filesystem );
				}
			}
			return $delete_done;
		}

		// staging（PR2 検証）へ出力（'all'）.
		foreach ( $suffix_map as $type => $suf ) {
			$stage_dir = $viewpv_dir . $type . '_new/';
			$this->stage_rawx_write( $stage_dir, $filename_base . $suf . '.php', $all[ $type ], $wp_filesystem );
		}

		// staging へ出力（tid 別・レンジは 'all' の filename_base を流用＝旧クォーク再現）.
		foreach ( $suffix_map as $type => $suf ) {
			foreach ( $t[ $type ] as $tracking_id => $arr ) {
				$stage_dir = $view_dir . $tracking_id . '/view_pv/' . $type . '_new/';
				$this->stage_rawx_write( $stage_dir, $filename_base . $suf . '.php', $arr, $wp_filesystem );
			}
		}
	...
	/**
	 * 既存日次 raw_x ファイル（指定日・指定 dir）を読み込む（過去日フォールバック供給用）。旧 L3703-3729 と同じ。
	 *
	 * @param string $dir    raw_X/ ディレクトリ.
	 * @param string $s_date 'YYYY-MM-DD'.
	 * @param object $wp_filesystem WP_Filesystem.
	 * @return array 日次 raw データ（無ければ空配列）.
	 */
	private function stage_rawx_load_daily( $dir, $s_date, $wp_filesystem ) {
		$data = array();
		$list = $this->wrap_dirlist( $dir );
		if ( ! $list ) {
			return $data;
		}
		for ( $i = 0, $max = $this->wrap_count( $list ); $i < $max; $i++ ) {
			if ( $this->wrap_substr( $list[ $i ]['name'], 0, 10 ) !== $s_date ) {
				continue;
			}
			$loaded = $this->wrap_unserialize( $this->wrap_get_contents( $dir . $list[ $i ]['name'] ) );
			if ( is_array( $loaded ) ) {
				$data = $loaded;
			}
		}
	...
	/**
	 * staging ファイルを書き出す（0 件は出さない＝旧 merge の wrap_count>0 条件と同値）。dir が無ければ作る。
	 *
	 * @param string $dir      出力 dir（raw_X_new/）.
	 * @param string $filename ファイル名.
	 * @param array  $data     書き出す配列.
	 * @param object $wp_filesystem WP_Filesystem.
	 * @return void
	 */
	private function stage_rawx_write( $dir, $filename, $data, $wp_filesystem ) {
		if ( $this->wrap_count( $data ) <= 0 ) {
			return;
		}
		if ( ! $wp_filesystem->exists( $dir ) ) {
			$wp_filesystem->mkdir( $dir );
		}
		$this->wrap_put_contents( $dir . $filename, $this->wrap_serialize( $data ) );
	}

	/**
	 * P7-D PR3 #1379: 指定日の本番 raw_x 日次ファイルを '全 tid（'all' 含む）× 全 type' で日付 glob 削除する。
	 *
	 * cutover ON で view_pv 経路は raw_x 旧日次を消さなくなる（C を skip）ため、独立ステップが「再作成の直前」に
	 * その日付の本番日次を一掃する。日付（先頭10桁）一致で消すので、drift で pv_id レンジが変わって別名になった
	 * 旧日次も orphan として残さない（ColumnDB の同日2ファイル二重計測を根絶）。source の read は呼び出し側で
	 * 先に完了しているので self-clobber しない。掃除対象 tid はディスク実在の view/ 配下から列挙（'all' もその1つ）。
	 *
	 * @param string $view_dir data/view/.
	 * @param string $s_date   'YYYY-MM-DD'.
	 * @param object $wp_filesystem WP_Filesystem.
	 * @return void
	 */
	private function stage_rawx_delete_daily_for_date( $view_dir, $s_date, $wp_filesystem ) {
		$types    = array( 'raw_p', 'raw_c', 'raw_e', 'raw_g' );
		$tid_dirs = $this->wrap_dirlist( $view_dir );
		if ( ! $tid_dirs ) {
			return;
		...
		foreach ( $tid_dirs as $tid_entry ) {
			...
			foreach ( $types as $type ) {
				...
				foreach ( $files as $file ) {
					// 日付（先頭10桁）一致の本番日次のみ削除（旧名・別レンジ名も対象）。
					if ( $this->wrap_substr( $file['name'], 0, 10 ) !== $s_date ) {
						continue;
					}
					if ( is_file( $dir . $file['name'] ) ) {
						$wp_filesystem->delete( $dir . $file['name'] );
	...
	/**
	 * P7-D PR3 #1379: 独立ステップが読んだ個別 -done を既存の削除リスト（del_rawfileslist_file）へ追記する。
	 *
	 * del_rawfileslist_file は write_ary_to_temp 形式（先頭=404 ヘッダ行・以降 1 行 1 パス）で、毎サイクル冒頭に
	 * _old1→_old2 と2日かけてローテーションし Night>Delete>Files>Raw で物理削除される。独立ステップは
	 * Merge delete file の後に走るので、この本体へ append すれば翌サイクルで _old1 化＝既存の遅延削除に自然に乗る
	 * （per-pid delete_rawfiles_ary_temp-*.php は preflight GC で消えるので使わない）。
	 *
	 * @param string $del_rawfileslist_file 削除リスト本体パス.
	 * @param array  $files 追記する個別 -done パス配列.
	 * @param object $wp_filesystem WP_Filesystem.
	 * @return void
	 */
	private function stage_rawx_append_delete_list( $del_rawfileslist_file, $files, $wp_filesystem ) {
		if ( empty( $files ) ) {
			return;
		}
		// 既存エントリ（ヘッダ行・空行を除外）を拾って結合し、write_ary_to_temp でヘッダごと再生成.
		$existing = array();
		if ( $wp_filesystem->exists( $del_rawfileslist_file ) ) {
			$lines = $wp_filesystem->get_contents_array( $del_rawfileslist_file );
			if ( is_array( $lines ) ) {
				foreach ( $lines as $line ) {
					...
					if ( false !== strpos( $line, 'http_response_code' ) ) {
						continue; // 404 ヘッダ行は write_ary_to_temp が再付与する.
					}
					$existing[] = $line;
				}
			}
		...
		$this->write_ary_to_temp( $merged, $del_rawfileslist_file );
	}
} // end of class
