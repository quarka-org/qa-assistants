...
class QAHM_ColumnDB_GscCron extends QAHM_Base {
	...
	private function get_unprocessed_dates( $tracking_id ) {
		...
		// ファイル名は cron-proc.php の暦日バケットで命名されるため、比較側の「今日」も同じ基準に
		// 揃える。#1153: 現状ファイル名は WP-TZ だが View_pv の per-site 化（別 PR）でサイトTZ命名へ
		// 移るので、ここも get_site_clock($tracking_id) のサイト暦に揃える（現 population は全サイト
		// WP-TZ なので挙動不変・ファイル名サイトTZ化で自動整合）。
	...
	private function convert_gsc_one_date( $tracking_id, $date_ymd ) {
		...
		if ( $processed === 0 && ! file_exists( $gsc_check ) ) {
			// 格納先ディレクトリは write_file() が dirname() に対して作成する。
