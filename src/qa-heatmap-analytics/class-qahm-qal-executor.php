...
class QAHM_Qal_Executor extends QAHM_File_Base {
	...
	private function qal_load_validation_manifest() {
		$manifest_file = dirname( __FILE__ ) . '/yaml/qal-validation-2026-05-11.php';
	...
	/**
	 * T87c: Extract material.column references from a view's calc map.
	 *
	 * Parses each calc expression's argument (the content inside the single
	 * pair of parentheses, e.g. "click_event.pv_id" from "COUNT(click_event.pv_id)")
	 * using the same strpos-based method already in use for calc validation
	 * (L716-763). Returns one entry per calc key.
	 *
	 * Output entries have shape:
	 *   [ 'calc_key' => string, 'calc_expr' => string,
	 *     'material' => string|null, 'column' => string|null ]
	 *
	 * material / column may be null when the expression is malformed; callers
	 * should rely on the earlier E_CALC_INVALID_EXPRESSION check to filter
	 * those out before reaching this point.
	 *
	 * @param array $calc_array view_def['calc'] map (key => "FUNC(material.column)")
	 * @param array $available_material_names Material prefixes to match against
	 *                                        (longer-first match to handle dotted names like "events.purchase")
	 * @return array List of { calc_key, calc_expr, material, column } entries
	 */
	private function extract_calc_column_refs( $calc_array, $available_material_names ) {
		$refs = array();
		if ( ! is_array( $calc_array ) ) {
			return $refs;
		}

		foreach ( $calc_array as $calc_key => $calc_expr ) {
			...
			$material = null;
			$column   = null;
			foreach ( $available_material_names as $name ) {
				$prefix = $name . '.';
				if ( strpos( $full_col, $prefix ) === 0 ) {
					$candidate_col = substr( $full_col, strlen( $prefix ) );
					if ( $candidate_col !== '' && $candidate_col !== false ) {
						// Longer prefix wins (e.g. "events.purchase" before "events")
						if ( $material === null || strlen( $name ) > strlen( $material ) ) {
							$material = $name;
	...
	private function qal_validate_make( $make, $rule, $materials ) {
		...
		foreach ( $make as $view_name => $view_def ) {
			$view_result = $this->qal_validate_view( $view_name, $view_def, $available_materials, $defined_views, $make );
			if ( $view_result !== true ) {
				return $view_result;
			}
		}
	...
	// =======================================================================
	// View-level validators (called from qal_validate_make per view)
	// =======================================================================

	/**
	 * Validate a single view definition (orchestrator).
	 *
	 * Calls each view-level validator in the same order as the pre-T88 inlined logic
	 * to preserve error_code precedence. Registers $view_name into $defined_views on
	 * success so subsequent views can reference it via from.
	 *
	 * @param string $view_name
	 * @param array  $view_def
	 * @param array  $available_materials
	 * @param array  $defined_views (passed by reference; appended on success)
	 * @param array  $make Full make object (passed through to join validator)
	 * @return true|array
	 */
	private function qal_validate_view( $view_name, $view_def, $available_materials, &$defined_views, $make ) {
		$shape_result = $this->qal_validate_view_shape( $view_name, $view_def );
		if ( $shape_result !== true ) {
			return $shape_result;
		}

		$from_result = $this->qal_validate_view_from( $view_name, $view_def, $available_materials, $defined_views );
		if ( is_array( $from_result ) ) {
			return $from_result;
		}
		$from_material = $from_result;

		$no_alias_result = $this->qal_validate_view_no_alias( $view_name, $view_def );
		if ( $no_alias_result !== true ) {
			return $no_alias_result;
		}

		$keep_result = $this->qal_validate_view_keep( $view_name, $view_def, $available_materials, $defined_views );
		if ( $keep_result !== true ) {
			return $keep_result;
		}

		$calc_result = $this->qal_validate_view_calc( $view_name, $view_def, $available_materials, $defined_views, $from_material );
		if ( $calc_result !== true ) {
			return $calc_result;
		}

		$guard_result = $this->qal_validate_view_filter_chain_guard( $view_name, $view_def, $defined_views );
		...
		if ( isset( $view_def['filter'] ) ) {
			...
			// T78d: pass from / join.with material names so E_FILTER_INVALID can hint
			// the user to swap from↔join.with when they're filtering on the wrong side.
			$join_with_material = isset( $view_def['join']['with'] ) && is_string( $view_def['join']['with'] ) ? $view_def['join']['with'] : null;
			$filter_result = $this->qal_validate_filter( $view_def['filter'], $view_name, $available_materials, $defined_views, $valid_columns, $from_material, $join_with_material );
		...
		if ( isset( $view_def['join'] ) ) {
			// T87: pass $from_material so the validator can pinpoint on.left scope
			// against the actual material (resolving view chain when needed).
			$join_result = $this->qal_validate_join( $view_def['join'], $view_name, $available_materials, $defined_views, $make, $from_material );
			if ( $join_result !== true ) {
				return $join_result;
			}
		}
		...
		if ( isset( $view_def['sort'] ) ) {
			$sort_result = $this->qal_validate_view_sort( $view_name, $view_def );
			if ( $sort_result !== true ) {
				return $sort_result;
			}
		}
	...
	/**
	 * Validate the structural shape of a view: view name pattern, view_def is an object,
	 * and required keys "from" / "keep" are present.
	 *
	 * @param string $view_name
	 * @param array  $view_def
	 * @return true|array
	 */
	private function qal_validate_view_shape( $view_name, $view_def ) {
	...
	/**
	 * Validate the from clause: array, single element, references a known material
	 * or previously defined view. Returns the resolved from_material on success.
	 *
	 * @param string $view_name
	 * @param array  $view_def
	 * @param array  $available_materials
	 * @param array  $defined_views
	 * @return string|array from_material on success, error array on failure
	 */
	private function qal_validate_view_from( $view_name, $view_def, $available_materials, $defined_views ) {
	...
	/**
	 * V1: forbid the "as" key in view definitions. Kept separate from shape because
	 * its evaluation order is after from-check in the original inlined logic.
	 *
	 * @param string $view_name
	 * @param array  $view_def
	 * @return true|array
	 */
	private function qal_validate_view_no_alias( $view_name, $view_def ) {
	...
	/**
	 * Validate keep: must be an array (empty allowed only with calc), each element
	 * is a string with no expression and resolves to a known qualified column.
	 * Also auto-derives view_def['add'] from calc keys when add is omitted; the
	 * write-back propagates to the calc validator via the &$view_def reference.
	 *
	 * @param string $view_name
	 * @param array  &$view_def (auto-derive add writes back here)
	 * @param array  $available_materials
	 * @param array  $defined_views
	 * @return true|array
	 */
	private function qal_validate_view_keep( $view_name, &$view_def, $available_materials, $defined_views ) {
	...
	/**
	 * Validate calc: add/calc subset symmetry, function whitelist, expression shape,
	 * and T87c view-scope column resolution against the manifest schema.
	 * Only the T87c view-scope column resolution step is skipped when from is a
	 * defined view (view chain); the add/calc symmetry and expression/function
	 * checks always run.
	 *
	 * @param string $view_name
	 * @param array  $view_def
	 * @param array  $available_materials
	 * @param array  $defined_views
	 * @param string $from_material
	 * @return true|array
	 */
	private function qal_validate_view_calc( $view_name, $view_def, $available_materials, $defined_views, $from_material ) {
		$has_calc = isset( $view_def['calc'] ) && is_array( $view_def['calc'] ) && ! empty( $view_def['calc'] );
		if ( ! $has_calc ) {
			return true;
		}
		...
		// T87c: E_CALC_COLUMN_UNRESOLVED -- after format validation passes,
		// verify each calc reference resolves against this view's scope
		// (from + join.with) and the referenced column exists in the
		// material's manifest schema.
		//
		// View chain (from[0] is a defined view): per design §4.8, calc
		// symmetry over view chains is future work. Pre-validating
		// material.column references here would incorrectly reject
		// runtime-resolvable references like COUNT(allpv.pv_id) when
		// from is a view whose underlying material is allpv. The
		// Material layer resolves the underlying material at runtime
		// (qal-material L1474-1478), so we skip this pre-check entirely
		// for view-chain views.
		$is_view_chain_calc = $this->wrap_in_array( $from_material, $defined_views, true );
		if ( ! $is_view_chain_calc ) {
		$view_scope = array( $from_material );
		if ( isset( $view_def['join']['with'] ) && is_string( $view_def['join']['with'] ) ) {
			$view_scope[] = $view_def['join']['with'];
		}
		// Longer prefix first so dotted material names match correctly
		usort( $view_scope, function ( $a, $b ) {
			return strlen( $b ) - strlen( $a );
		} );

		$calc_refs = $this->extract_calc_column_refs( $view_def['calc'], $view_scope );
		foreach ( $calc_refs as $ref ) {
			// If material is null here, the prefix did not match any view-scope
			// material. The reference might still be a defined view (view chain)
			// which we leave to Material layer; otherwise it's unresolved.
			if ( $ref['material'] === null ) {
				$is_view_ref = false;
				foreach ( $defined_views as $dv ) {
					$prefix = $dv . '.';
					$paren_open_e  = strpos( $ref['calc_expr'], '(' );
					$paren_close_e = strpos( $ref['calc_expr'], ')' );
					if ( $paren_open_e !== false && $paren_close_e !== false ) {
						$expr_col = substr( $ref['calc_expr'], $paren_open_e + 1, $paren_close_e - $paren_open_e - 1 );
						if ( strpos( $expr_col, $prefix ) === 0 ) {
							$is_view_ref = true;
							break;
						}
					}
				...
				// Extract referenced material name (best-effort) for the details payload
				$paren_open_e  = strpos( $ref['calc_expr'], '(' );
				$paren_close_e = strpos( $ref['calc_expr'], ')' );
				$ref_material  = '';
				if ( $paren_open_e !== false && $paren_close_e !== false ) {
					$expr_col   = substr( $ref['calc_expr'], $paren_open_e + 1, $paren_close_e - $paren_open_e - 1 );
					$dot_pos    = strpos( $expr_col, '.' );
					$ref_material = ( $dot_pos !== false ) ? substr( $expr_col, 0, $dot_pos ) : $expr_col;
				}
				return array(
					'valid' => false,
					'error_code' => 'E_CALC_COLUMN_UNRESOLVED',
					'message' => 'calc references a material that is not in the view scope (from / join.with).',
					'location' => "make.{$view_name}.calc.{$ref['calc_key']}",
					'details' => array(
						'calc_key'           => $ref['calc_key'],
						'calc_expr'          => $ref['calc_expr'],
						'referenced_material' => $ref_material,
						'available_materials' => $view_scope,
					)
				);
			}

			$manifest_columns = $this->get_material_column_names( $ref['material'] );
			if ( empty( $manifest_columns ) ) {
				// Manifest itself missing -- skip; other checks already cover this
				continue;
			}
			if ( ! isset( $manifest_columns[ $ref['column'] ] ) ) {
				$available_columns = array_keys( $manifest_columns );
				if ( count( $available_columns ) > 20 ) {
					$available_columns = array_slice( $available_columns, 0, 20 );
				}
				return array(
					'valid' => false,
					'error_code' => 'E_CALC_COLUMN_UNRESOLVED',
					'message' => 'calc references a column that does not exist in the material schema.',
					'location' => "make.{$view_name}.calc.{$ref['calc_key']}",
					'details' => array(
						'calc_key'           => $ref['calc_key'],
						'calc_expr'          => $ref['calc_expr'],
						'referenced_material' => $ref['material'],
	 ...
	 * @return true|array
	 */
	private function qal_validate_view_filter_chain_guard( $view_name, $view_def, $defined_views ) {
		// T32: view chain + filter is not supported (HAVING equivalent is future work)
	...
	/**
	 * T65: validate sort -- object shape, required by/order, optional positive top,
	 ...
	 * @return true|array
	 */
	private function qal_validate_view_sort( $view_name, $view_def ) {
	 ...
	 * @param string|null $from_material Name of the "from" material (T78d: surfaced in E_FILTER_INVALID hints).
	 * @param string|null $join_with_material Name of the join.with target if any (T78d: shown as a swap suggestion).
	 * @return true|array True if valid, error array otherwise
	 */
	private function qal_validate_filter( $filter, $view_name, $available_materials, $defined_views, $valid_columns = array(), $from_material = null, $join_with_material = null ) {
		...
		foreach ( $filter as $column => $condition ) {
			...
			if ( ! empty( $valid_columns ) && ! isset( $valid_columns[ $column ] ) ) {
				// T78d: surface from material + the swap-with-join hint so AI / human
				// callers can self-correct without re-reading the materials manifest.
				$details = array(
					'received_key' => $column,
					'valid_columns' => array_keys( $valid_columns ),
					'examples' => $filter_examples,
				);
				$message = 'Filter key "' . $column . '" is not a valid column name';
				if ( $from_material !== null && $from_material !== '' ) {
					$details['from_material'] = $from_material;
					$message .= ' for material "' . $from_material . '"';
				}
				$message .= '.';
				if ( $join_with_material !== null && $join_with_material !== '' ) {
					$details['available_join_with'] = $join_with_material;
					$details['hint'] = "Try swapping 'from' to '" . $join_with_material . "' if you want to filter by '" . $column . "'. (filter only applies to the 'from' material's columns.)";
				}
				return array(
					'valid' => false,
					'error_code' => 'E_FILTER_INVALID',
	 ...
	 * @param string|null $from_material from[0] of the current view (used to pinpoint on.left scope)
	 * @return true|array True if valid, error array otherwise
	 */
	private function qal_validate_join( $join, $view_name, $available_materials, $defined_views, $make = array(), $from_material = null ) {
		if ( ! is_array( $join ) ) {
			return array(
				'valid' => false,
		...
		// T87: resolve left scope once before the on loop.
		// If $from_material is a defined view, walk the view chain (max 10 hops,
		// using the same procedure as the join.with view-chain resolution further down)
		// until we land on a real material; otherwise use $from_material as-is.
		// This becomes the only allowed prefix for on.left.
		$resolved_left_scope = null;
		if ( is_string( $from_material ) && $from_material !== '' ) {
			if ( $this->wrap_in_array( $from_material, $available_materials, true ) ) {
				$resolved_left_scope = $from_material;
			} elseif ( $this->wrap_in_array( $from_material, $defined_views, true ) ) {
				$current = $from_material;
				$depth = 0;
		...
		foreach ( $join['on'] as $on_index => $on_pair ) {
			...
			// T87: pinpoint scope check — on.left must be prefixed with the resolved
			// left scope (real material name, view chain resolved). View-name prefixes
			// are intentionally not allowed (§6.4) so pre-validation matches the
			// execution layer behavior in qal_join_apply / parse_column_ref.
			if ( $resolved_left_scope !== null ) {
				$left_prefix = $resolved_left_scope . '.';
				if ( strpos( $on_pair['left'], $left_prefix ) !== 0
					|| ! $this->is_alnum_underscore( substr( $on_pair['left'], strlen( $left_prefix ) ) ) ) {
					$hint = "change '" . $on_pair['left'] . "' to '" . $left_prefix . "<column>'";
					return array(
						'valid' => false,
						'error_code' => 'E_INVALID_JOIN',
						'message' => 'Join on.left must reference the from material "' . $resolved_left_scope . '". Got: ' . $on_pair['left'],
						'location' => "make.{$view_name}.join.on[{$on_index}].left",
						'details' => array(
							'side' => 'left',
							'received_value' => $on_pair['left'],
							'expected_prefix' => $left_prefix,
							'hint' => $hint,
						)
					);
				}
			...
			// T87: pinpoint scope check — on.right must be prefixed with join.with
			// itself (view name OR material name, exactly as written in `with`).
			// Walking the view chain on the right side is intentionally NOT done
			// (§6.5) so pre-validation matches parse_column_ref behavior.
			$right_prefix = $with . '.';
			if ( strpos( $on_pair['right'], $right_prefix ) !== 0
				|| ! $this->is_alnum_underscore( substr( $on_pair['right'], strlen( $right_prefix ) ) ) ) {
				$hint = "change '" . $on_pair['right'] . "' to '" . $right_prefix . "<column>'";
				return array(
					'valid' => false,
					'error_code' => 'E_INVALID_JOIN',
					'message' => 'Join on.right must reference join.with "' . $with . '". Got: ' . $on_pair['right'],
					'location' => "make.{$view_name}.join.on[{$on_index}].right",
					'details' => array(
						'side' => 'right',
						'received_value' => $on_pair['right'],
						'expected_prefix' => $right_prefix,
						'hint' => $hint,
					)
				);
			}
	...
	public function qal_build_response( $executable_qal ) {
		...
		$view_def = isset( $qal['make'][ $view_name ] ) ? $qal['make'][ $view_name ] : array();
		$material_names = array();
		if ( isset( $qal['materials'] ) && is_array( $qal['materials'] ) ) {
			foreach ( $qal['materials'] as $m ) {
				if ( isset( $m['name'] ) ) {
					$material_names[] = $m['name'];
				}
			}
		}
		if ( isset( $qal['make'] ) && is_array( $qal['make'] ) ) {
			foreach ( array_keys( $qal['make'] ) as $vn ) {
				$material_names[] = $vn;
			}
		}
		$material_names = array_values( array_unique( $material_names ) );
		$data = $this->apply_final_strip( $view_def, $data, $material_names );
	...
	/**
	 * Apply final-stage strip to rows: drop columns not in keep + add (calc/add outputs).
	 *
	 * Strip is the sole responsibility of the executor's final stage. Middle stages
	 * (material merge, executor view chain entry) intentionally retain internal columns
	 * so symmetric joins and downstream view-chain calc references stay readable.
	 *
	 * @param array $view_def Final view definition (with keep / add / calc)
	 * @param array $rows Rows after all view operations
	 * @param array $material_names Names used as qualifier prefixes in keep (materials + view names)
	 * @return array Rows with only allowed keys preserved
	 */
	private function apply_final_strip( $view_def, $rows, $material_names ) {
		if ( empty( $rows ) ) {
			return $rows;
		}

		$allow_set = array();
		if ( ! empty( $view_def['keep'] ) && is_array( $view_def['keep'] ) ) {
			foreach ( $view_def['keep'] as $qualified_col ) {
				$short = $this->strip_material_prefix( $qualified_col, $material_names );
				$allow_set[ $short ] = true;
			}
		}
		if ( ! empty( $view_def['add'] ) && is_array( $view_def['add'] ) ) {
			foreach ( $view_def['add'] as $add_col ) {
				$allow_set[ $add_col ] = true;
			}
		}
		if ( ! empty( $view_def['calc'] ) && is_array( $view_def['calc'] ) ) {
			foreach ( array_keys( $view_def['calc'] ) as $calc_key ) {
				$allow_set[ $calc_key ] = true;
			}
		}
		...
		$stripped = array();
		foreach ( $rows as $row ) {
			$stripped[] = array_intersect_key( $row, $allow_set );
		}
		return $stripped;
	}
