 ...
 * Edit materials-manifest.yaml and run:
 *   php scripts/build-manifest.php materials
 *
 * Generated: 2026-08-05 02:22:18
 *
...
',
	...
	'materials' => array(
				...
				array(
					...
					'fields' => array(
						...
						array(
							...
							'semantics' => 'id',
							'search_hint' => array(
								'note' => 'セッション単位の絞り込みに使用。列DBバイナリスキャンで高速。値は YYYYMM × 10^9 + month_seq の合成 uint64 で月内連番（月をまたぐとリセット）',
							),
						),
						array(
						...
						array(
							...
							'semantics' => 'id',
							'search_hint' => array(
								'note' => '3〜5種（1=PC, 2=tablet, 3=SP）。列DBバイナリスキャンで高速。人間向けにはdevice_typeを使う',
							),
						...
						array(
							...
							'status' => 'dev',
							'search_hint' => array(
								'note' => 'フォーム送信フラグ(0/1)。ネイティブ submit イベント観測由来(Enter送信も含む・action_id 非依存)のPV単位フラグ。eq:1で実際にフォーム送信が発火したPVを抽出。page_idと組み合わせてフォームCVR算出に使う',
							),
						),
						array(
						...
						array(
							'material_column' => 'window_inner_width',
							'physical_column' => 'window_inner_width',
							'type' => 'uint16',
							'nullable' => true,
							'default' => 0,
							'status' => 'dev',
							'search_hint' => array(
								'note' => 'ブラウザのwindow.innerWidth(px)。実表示コンテンツ領域の横幅。0はraw_eヘッダー欠落の可能性。gte:1で実値のみ抽出。between:768,1199でタブレット帯、gte:1200でPC帯のセグメント分析に使う',
							),
						),
						array(
							'material_column' => 'window_inner_height',
							'physical_column' => 'window_inner_height',
							'type' => 'uint16',
							'nullable' => true,
							'default' => 0,
							'status' => 'dev',
							'search_hint' => array(
								'note' => 'ブラウザのwindow.innerHeight(px)。実表示コンテンツ領域の縦幅。0はraw_eヘッダー欠落の可能性。gte:1で実値のみ抽出。window_inner_widthと組み合わせてビューポートサイズ別の挙動分析に使う',
							),
						),
						array(
						...
						array(
							...
							'since' => '2026-07-09',
							'search_hint' => array(
								'note' => 'スクロール生データ(raw_p)保有数(0/1/2…＝captures 数)。>0（neq:0）でヒートマップ対象PVを抽出（eq:1 だと is_raw≥2 を取りこぼす）。ヒートマップ生成が列DBだけで完結する土台',
							),
						),
						array(
							...
							'since' => '2026-07-09',
							'search_hint' => array(
								'note' => 'クリック生データ(raw_c)保有数(0/1/2…)。>0（neq:0）でクリックヒートマップ対象PVを抽出',
							),
						),
						array(
							...
							'since' => '2026-07-09',
							'search_hint' => array(
								'note' => 'イベント生データ(raw_e)保有数(0/1/2…)。>0（neq:0）でイベント記録のあるPVを抽出',
							),
						),
						array(
						...
						array(
							'material_column' => 'month',
							'physical_column' => 'month',
							'virtual' => true,
							'status' => 'dev',
							'since' => '2026-08-05',
							'search_hint' => array(
								'note' => '所属月（"YYYY-MM"）。access_time からサイト時計で算出（列DB経路・view_pv経路とも同一境界）。keep に入れると月別に集計できる（月次推移レポートの唯一の手段。QAL に日付を丸める calc は無い）。期間の絞り込みは time で行うこと（この列の filter は復号後の post_filter で遅い）',
							),
						),
						array(
				...
				array(
					...
					'fields' => array(
						...
						array(
							...
							'semantics' => 'id',
							'search_hint' => array(
								'note' => 'クリック対象の事実分類コード。1:click(汎用・リンク含む) / 2:form(クリックが<form>領域内で起きたDOM事実・フォーム摩擦や離脱の funnel 信号) / 3:tel / 4:mailto。相互排他で tel/mailto を form より優先。送信成否そのものは is_submit を、CVは goal(is_goal_x)を見る。to_url と併せれば 1:click のうちリンク遷移を判別できる',
							),
						),
						array(
				...
				array(
					...
					'fields' => array(
						...
						array(
							...
							'since' => '2026-08-05',
							'search_hint' => array(
								'note' => '所属月（"YYYY-MM"）。gsc の行は日付を持たないため、Storage が日別ファイルのループから注入した日付で算出する。keep に入れると月別に集計できる。期間の絞り込みは time で行うこと',
							),
						),
						array(
							'material_column' => 'ctr',
							...
							'semantics' => 'rate',
							'search_hint' => array(
								'note' => 'clicks / impressions。Material層で算出。filter は復号後の post_filter（大量行では遅い）',
							),
						),
						array(
							...
							'semantics' => 'rate',
							'search_hint' => array(
								'note' => 'position_x100 / 100。Material層で算出。filter は position_x100 にリライトされ列DBスキャン可能（速い）',
							),
						),
						array(
							...
							'status' => 'dev',
							'search_hint' => array(
								'note' => 'position × impressions。Material層で算出。加重平均position計算用の素材。filter は復号後の post_filter（大量行では遅い）',
							),
						),
					),
				),
				...
				array(
					...
					'fields' => array(
						array(
							...
							'semantics' => 'id',
							'search_hint' => array(
								'note' => 'PK / JOINキー。allpv.version_idとの結合用。直接指定よりallpv JOIN page_versionを推奨',
							),
						),
						array(
							'material_column' => 'page_id',
						...
						array(
							...
							'semantics' => 'id',
							'search_hint' => array(
								'note' => 'デバイス識別（1=PC, 2=tablet, 3=SP）',
							),
						),
						array(
							'material_column' => 'version_no',
		...
		'summary_landingpage' => array(
			'supports_all' => true,
			'decoders' => array(
				array(
					'loader' => 'db.summary_days_landingpages',
					'decoder' => NULL,
					'fields' => array(
						array(
							'material_column' => 'page_id',
							...
							'semantics' => 'id',
							'search_hint' => array(
								'note' => 'サマリー由来。入口ページ識別。ページ別の絞り込みは page_id で行う（page_url 部分一致は未対応、必要なら allpv）',
							),
						),
						array(
							'material_column' => 'device_id',
							...
							'semantics' => 'id',
							'search_hint' => array(
								'note' => 'サマリー由来。デバイス識別（1=PC, 2=tablet, 3=SP）。数値IDのまま。表示用ラベルは value_mappings 経由で device_type 列が自動付与される',
							),
							'value_mappings' => array(
								'label_column' => 'device_type',
								'map' => array(
						...
						array(
							...
							'status' => 'dev',
							'search_hint' => array(
								'note' => 'サマリー由来。流入元（ラベル文字列。loader 側でID解決済み）',
							),
						),
						array(
							'material_column' => 'utm_medium',
							...
							'status' => 'dev',
							'search_hint' => array(
								'note' => 'サマリー由来。流入メディア（ラベル文字列。loader 側でID解決済み）',
							),
						),
						array(
							'material_column' => 'utm_campaign',
							...
							'status' => 'dev',
							'search_hint' => array(
								'note' => 'サマリー由来。流入キャンペーン（ラベル文字列。loader 側でID解決済み）',
							),
						),
						array(
							'material_column' => 'source_domain',
							...
							'status' => 'dev',
							'search_hint' => array(
								'note' => 'サマリー由来。流入元ドメイン（広告トラフィックは loader 側で補完される）',
							),
						),
						array(
							'material_column' => 'is_newuser',
							...
							'status' => 'dev',
							'search_hint' => array(
								'note' => 'サマリー由来。新規ユーザーフラグ（1=新規, 0=リピート）',
							),
						),
						array(
							'material_column' => 'second_page',
							'physical_column' => 'second_page',
							'type' => 'uint32',
							'status' => 'dev',
							'semantics' => 'id',
							'search_hint' => array(
								'note' => 'サマリー由来。入口の次に閲覧したページID（2ページ目）。0は離脱',
							),
						),
						array(
							'material_column' => 'is_QA',
							'physical_column' => 'is_QA',
							'type' => 'uint8',
							'status' => 'dev',
							'search_hint' => array(
								'note' => 'サマリー由来。QAツール経由フラグ',
							),
						),
						array(
							'material_column' => 'pv_count',
							'physical_column' => 'pv_count',
							'type' => 'uint32',
							'status' => 'dev',
							'search_hint' => array(
								'note' => 'サマリー由来。指定期間のページビュー累計。数値範囲フィルタは post_filter',
							),
						),
						array(
							'material_column' => 'session_count',
							'physical_column' => 'session_count',
							'type' => 'uint32',
							'status' => 'dev',
							'search_hint' => array(
								'note' => 'サマリー由来。指定期間のセッション累計。数値範囲フィルタは post_filter',
							),
						),
						array(
							'material_column' => 'user_count',
							'physical_column' => 'user_count',
							'type' => 'uint32',
							'status' => 'dev',
							'search_hint' => array(
								'note' => 'サマリー由来。指定期間のユーザー累計。数値範囲フィルタは post_filter',
							),
						),
						array(
							'material_column' => 'bounce_count',
							'physical_column' => 'bounce_count',
							'type' => 'uint32',
							'status' => 'dev',
							'search_hint' => array(
								'note' => 'サマリー由来。指定期間の直帰数累計。数値範囲フィルタは post_filter',
							),
						),
						array(
							'material_column' => 'session_time',
							'physical_column' => 'session_time',
							'type' => 'uint32',
							'status' => 'dev',
							'search_hint' => array(
								'note' => 'サマリー由来。指定期間の滞在時間合計（秒）。数値範囲フィルタは post_filter',
							),
						),
						array(
							'material_column' => 'title',
							...
							'status' => 'dev',
							'search_hint' => array(
								'note' => 'サマリー由来。入口ページのタイトル（loader 側で page_id から復元）',
							),
						),
						array(
							'material_column' => 'url',
							...
							'status' => 'dev',
							'search_hint' => array(
								'note' => 'サマリー由来。入口ページのURL（loader 側で page_id から復元）',
							),
						),
						array(
							'material_column' => 'wp_qa_id',
							'physical_column' => 'wp_qa_id',
							'status' => 'dev',
							'semantics' => 'id',
							'search_hint' => array(
								'note' => 'サマリー由来。WordPress 投稿ID（loader 側で page_id から復元）',
							),
						),
					),
				),
		...
		'summary_allpage' => array(
			'supports_all' => true,
			'decoders' => array(
				array(
					'loader' => 'db.summary_days_allpages',
					'decoder' => NULL,
					'fields' => array(
						array(
							'material_column' => 'page_id',
							...
							'semantics' => 'id',
							'search_hint' => array(
								'note' => 'サマリー由来。ページ識別。ページ別ランキングはこのマテリアル。page_url 部分一致は未対応、必要なら allpv',
							),
						),
						array(
							'material_column' => 'device_id',
						...
						array(
							'material_column' => 'exit_count',
							'physical_column' => 'exit_count',
							'type' => 'uint32',
							'status' => 'dev',
							'search_hint' => array(
								'note' => 'サマリー由来。指定期間の離脱数累計。数値範囲フィルタは post_filter',
							),
						),
						array(
							'material_column' => 'time_on_page',
							'physical_column' => 'time_on_page',
							'type' => 'uint32',
							'status' => 'dev',
							'search_hint' => array(
								'note' => 'サマリー由来。指定期間の滞在時間合計（秒）。数値範囲フィルタは post_filter',
						...
						array(
							'material_column' => 'lp_count',
							'physical_column' => 'lp_count',
							'type' => 'uint32',
							'status' => 'dev',
							'search_hint' => array(
								'note' => 'サマリー由来。指定期間に入口（ランディングページ）になった回数の累計',
							),
						),
						array(
							'material_column' => 'title',
							...
							'status' => 'dev',
							'search_hint' => array(
								'note' => 'サマリー由来。ページのタイトル（loader 側で page_id から復元）',
							),
						),
						array(
							'material_column' => 'url',
							...
							'status' => 'dev',
							'search_hint' => array(
								'note' => 'サマリー由来。ページのURL（loader 側で page_id から復元）',
							),
						),
						array(
							'material_column' => 'wp_qa_id',
		...
		'summary_days_access_detail' => array(
			'supports_all' => true,
			'decoders' => array(
				array(
					'loader' => 'db.summary_days_access_detail',
					'decoder' => NULL,
					'fields' => array(
						array(
							'material_column' => 'device_id',
						...
						array(
							'material_column' => 'is_newuser_count',
							'physical_column' => 'is_newuser_count',
							'type' => 'uint32',
							'status' => 'dev',
							'search_hint' => array(
								'note' => 'サマリー由来。指定期間の新規ユーザー数累計（このマテリアル固有のカラム）',
							),
						),
					),
				),
